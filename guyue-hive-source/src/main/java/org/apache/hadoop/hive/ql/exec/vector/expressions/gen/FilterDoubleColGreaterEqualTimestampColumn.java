 ...
 */
public class FilterDoubleColGreaterEqualTimestampColumn extends VectorExpression {
  ...
  public FilterDoubleColGreaterEqualTimestampColumn(int colNum1, int colNum2) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
  }

  public FilterDoubleColGreaterEqualTimestampColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (n != 0 && inputColVector1.isRepeating && inputColVector2.isRepeating) {
      ...
      if (!(vector1[0] >= inputColVector2.getDouble(0))) {
        batch.size = 0;
      }
    } else if (inputColVector1.isRepeating) {
      double value1 = vector1[0];
      if (batch.selectedInUse) {
        newSize = 0;
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          if (value1 >= inputColVector2.getDouble(i)) {
            sel[newSize++] = i;
          }
        }
        batch.size = newSize;
      ...
      for(int j = 0; j != n; j++) {
        int i = sel[j];
        if (vector1[i] >= inputColVector2.getDouble(i)) {
          sel[newSize++] = i;
        }
      }
      batch.size = newSize;

...
public class FilterVarCharColumnBetweenDynamicValue extends FilterVarCharColumnBetween {
  ...
  public void evaluate(VectorizedRowBatch batch) {
    if (!initialized) {
      ...
      if (lVal == null || rVal == null) {
        ...
        byte[] min = (PrimitiveObjectInspectorUtils.getHiveVarchar(
            lVal, leftDynamicValue.getObjectInspector()).getValue().getBytes());
        setLeftValue(min);

        byte[] max = (PrimitiveObjectInspectorUtils.getHiveVarchar(
            rVal, rightDynamicValue.getObjectInspector()).getValue().getBytes());
        setRightValue(max);
      }
      initialized = true;
    }

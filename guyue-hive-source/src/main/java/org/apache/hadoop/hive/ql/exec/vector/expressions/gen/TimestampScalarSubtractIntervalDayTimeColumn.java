 ...
 */
public class TimestampScalarSubtractIntervalDayTimeColumn extends VectorExpression {
  ...
  public TimestampScalarSubtractIntervalDayTimeColumn(Timestamp value, int colNum, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
    this.outputColumn = outputColumn;
  }

  public TimestampScalarSubtractIntervalDayTimeColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector2.isRepeating) {
      dtm.subtract(
          value, inputColVector2.asScratchIntervalDayTime(0), outputColVector.getScratchTimestamp());
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.subtract(
              value, inputColVector2.asScratchIntervalDayTime(i), outputColVector.getScratchTimestamp());

...
public class FuncCeilDecimalToDecimal extends VectorExpression {
  private static final long serialVersionUID = 1L;

  private int colNum;
  private int outputColumn;

  public FuncCeilDecimalToDecimal(int colNum, int outputColumn) {
    this();
    this.colNum = colNum;
    this.outputColumn = outputColumn;
  }
  
  public FuncCeilDecimalToDecimal() {
    super();
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      DecimalUtil.ceiling(0, vector[0], outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          ...
          DecimalUtil.ceiling(i, vector[i], outputColVector);
        }
      } else {

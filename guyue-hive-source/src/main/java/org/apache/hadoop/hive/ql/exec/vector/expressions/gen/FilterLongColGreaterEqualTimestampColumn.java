 ...
 */
public class FilterLongColGreaterEqualTimestampColumn extends VectorExpression {
  ...
  public FilterLongColGreaterEqualTimestampColumn(int colNum1, int colNum2) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
  }

  public FilterLongColGreaterEqualTimestampColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (n != 0 && inputColVector1.isRepeating && inputColVector2.isRepeating) {
      ...
      if (!(vector1[0] >= inputColVector2.getTimestampAsLong(0))) {
        batch.size = 0;
      }
      ...
      if (batch.selectedInUse) {
        ...
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          if (value1 >= inputColVector2.getTimestampAsLong(i)) {
            sel[newSize++] = i;
          }
        }
        batch.size = newSize;
      ...
      for(int j = 0; j != n; j++) {
        int i = sel[j];
        if (vector1[i] >= inputColVector2.getTimestampAsLong(i)) {
          sel[newSize++] = i;
        }
      }
      batch.size = newSize;

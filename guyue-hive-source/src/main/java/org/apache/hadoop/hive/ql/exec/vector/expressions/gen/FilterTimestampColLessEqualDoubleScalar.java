 ...
 */
public class FilterTimestampColLessEqualDoubleScalar extends VectorExpression {
  ...
  public FilterTimestampColLessEqualDoubleScalar(int colNum, double value) {
    this.colNum = colNum;
    this.value = value;
  }

  public FilterTimestampColLessEqualDoubleScalar() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.noNulls) {
      if (inputColVector.isRepeating) {
        ...
        if (!(inputColVector.getDouble(0) <= value)) {
        ...
        for(int j=0; j != n; j++) {
          int i = sel[j];
          if (inputColVector.getDouble(i) <= value) {
            sel[newSize++] = i;
          }
        }
        batch.size = newSize;

 ...
 */
public class DecimalScalarDivideDecimalColumn extends VectorExpression {
  ...
  public DecimalScalarDivideDecimalColumn(HiveDecimal value, int colNum, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      DecimalUtil.divideChecked(0, value, vector[0], outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.divideChecked(i, value, vector[i], outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.divideChecked(i, value, vector[i], outputColVector);

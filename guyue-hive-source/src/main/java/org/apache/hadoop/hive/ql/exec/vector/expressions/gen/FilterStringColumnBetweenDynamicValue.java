...
public class FilterStringColumnBetweenDynamicValue extends FilterStringColumnBetween {

  private static final long serialVersionUID = 1L;

  private static final Logger LOG = LoggerFactory.getLogger(FilterStringColumnBetweenDynamicValue.class);
  ...
  public FilterStringColumnBetweenDynamicValue(int colNum, DynamicValue leftValue, DynamicValue rightValue) {
    super(colNum, null, null);
    this.leftDynamicValue = leftValue;
    this.rightDynamicValue = rightValue;
  }

  public FilterStringColumnBetweenDynamicValue() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    if (!initialized) {
      ...
      if (lVal == null || rVal == null) {
        ...
        byte[] min = (PrimitiveObjectInspectorUtils.getString(
            lVal, leftDynamicValue.getObjectInspector()).getBytes());
        setLeftValue(min);

        byte[] max = (PrimitiveObjectInspectorUtils.getString(
            rVal, rightDynamicValue.getObjectInspector()).getBytes());
        setRightValue(max);
      }
      initialized = true;
    }

 ...
 */
public class IntervalDayTimeColGreaterEqualIntervalDayTimeColumn extends VectorExpression {
  ...
  public IntervalDayTimeColGreaterEqualIntervalDayTimeColumn(int colNum1, int colNum2, int outputColumn) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
    this.outputColumn = outputColumn;
  }

  public IntervalDayTimeColGreaterEqualIntervalDayTimeColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) {
      outputVector[0] = inputColVector1.compareTo(0, inputColVector2.asScratchIntervalDayTime(0)) >= 0 ? 1 : 0;
    } else if (inputColVector1.isRepeating) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = inputColVector1.compareTo(0, inputColVector2.asScratchIntervalDayTime(i)) >= 0 ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = inputColVector1.compareTo(0, inputColVector2.asScratchIntervalDayTime(i)) >= 0 ? 1 : 0;
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = inputColVector1.compareTo(i, value2) >= 0 ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = inputColVector1.compareTo(i, value2) >= 0 ? 1 : 0;
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = inputColVector1.compareTo(i, inputColVector2.asScratchIntervalDayTime(i)) >= 0 ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = inputColVector1.compareTo(i, inputColVector2.asScratchIntervalDayTime(i)) >= 0 ? 1 : 0;

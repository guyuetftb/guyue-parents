 ...
 */
public class DecimalColSubtractDecimalScalar extends VectorExpression {
  ...
  public DecimalColSubtractDecimalScalar(int colNum, HiveDecimal value, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
  ...
  public DecimalColSubtractDecimalScalar() {
    this.outputType = "decimal";
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      DecimalUtil.subtractChecked(0, vector[0], value, outputColVector);
    } else if (inputColVector.noNulls) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.subtractChecked(i, vector[i], value, outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.subtractChecked(i, vector[i], value, outputColVector);

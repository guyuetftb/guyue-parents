 ...
 */
public class TimestampColSubtractTimestampColumn extends VectorExpression {
  ...
  public TimestampColSubtractTimestampColumn(int colNum1, int colNum2, int outputColumn) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
    this.outputColumn = outputColumn;
  }

  public TimestampColSubtractTimestampColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) {
      dtm.subtract(
          inputColVector1.asScratchTimestamp(0), inputColVector2.asScratchTimestamp(0), outputColVector.getScratchIntervalDayTime());
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.subtract(
              value1, inputColVector2.asScratchTimestamp(i), outputColVector.getScratchIntervalDayTime());
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.subtract(
              inputColVector1.asScratchTimestamp(i), value2, outputColVector.getScratchIntervalDayTime());
          outputColVector.setFromScratchIntervalDayTime(i);
        }
      } else {
        for(int i = 0; i != n; i++) {
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.subtract(
              inputColVector1.asScratchTimestamp(i), inputColVector2.asScratchTimestamp(i), outputColVector.getScratchIntervalDayTime());
          outputColVector.setFromScratchIntervalDayTime(i);
        }
      } else {
        for(int i = 0; i != n; i++) {

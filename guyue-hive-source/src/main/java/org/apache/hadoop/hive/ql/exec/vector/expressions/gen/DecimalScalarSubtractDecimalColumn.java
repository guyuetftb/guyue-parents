 ...
 */
public class DecimalScalarSubtractDecimalColumn extends VectorExpression {
  ...
  public DecimalScalarSubtractDecimalColumn(HiveDecimal value, int colNum, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
  ...
  public DecimalScalarSubtractDecimalColumn() {
    this.outputType = "decimal";
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      DecimalUtil.subtractChecked(0, value, vector[0], outputColVector);
    } else if (inputColVector.noNulls) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.subtractChecked(i, value, vector[i], outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.subtractChecked(i, value, vector[i], outputColVector);

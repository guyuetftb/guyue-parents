...
public class FuncLog10DoubleToDouble extends VectorExpression {
  private static final long serialVersionUID = 1L;

  private int colNum;
  private int outputColumn;

  public FuncLog10DoubleToDouble(int colNum, int outputColumn) {
    this();
    this.colNum = colNum;
    this.outputColumn = outputColumn;
  }
  
  public FuncLog10DoubleToDouble() {
    super();
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      outputVector[0] =  Math.log10( vector[0]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] =  Math.log10( vector[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] =  Math.log10( vector[i]);

...
public class FuncFloorDecimalToDecimal extends VectorExpression {
  private static final long serialVersionUID = 1L;

  private int colNum;
  private int outputColumn;

  public FuncFloorDecimalToDecimal(int colNum, int outputColumn) {
    this();
    this.colNum = colNum;
    this.outputColumn = outputColumn;
  }
  
  public FuncFloorDecimalToDecimal() {
    super();
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      DecimalUtil.floor(0, vector[0], outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          ...
          DecimalUtil.floor(i, vector[i], outputColVector);
        }
      } else {

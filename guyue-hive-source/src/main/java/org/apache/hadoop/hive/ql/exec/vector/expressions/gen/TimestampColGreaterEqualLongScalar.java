 ...
 */
public class TimestampColGreaterEqualLongScalar extends VectorExpression {
  ...
  public TimestampColGreaterEqualLongScalar(int colNum, long value, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
    this.outputColumn = outputColumn;
  }

  public TimestampColGreaterEqualLongScalar() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.noNulls) {
      if (inputColVector.isRepeating) {
        ...
        outputVector[0] = inputColVector.getTimestampAsLong(0) >= value ? 1 : 0;
        outputColVector.isRepeating = true;
      } else if (batch.selectedInUse) {
        for(int j=0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = inputColVector.getTimestampAsLong(i) >= value ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = inputColVector.getTimestampAsLong(i) >= value ? 1 : 0;

...
public class FuncSqrtDoubleToDouble extends VectorExpression {
  private static final long serialVersionUID = 1L;

  private int colNum;
  private int outputColumn;

  public FuncSqrtDoubleToDouble(int colNum, int outputColumn) {
    this();
    this.colNum = colNum;
    this.outputColumn = outputColumn;
  }
  
  public FuncSqrtDoubleToDouble() {
    super();
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      outputVector[0] =  Math.sqrt( vector[0]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] =  Math.sqrt( vector[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] =  Math.sqrt( vector[i]);

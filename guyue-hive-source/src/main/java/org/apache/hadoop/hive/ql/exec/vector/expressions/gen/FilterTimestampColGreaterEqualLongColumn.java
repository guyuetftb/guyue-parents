 ...
 */
public class FilterTimestampColGreaterEqualLongColumn extends VectorExpression {
  ...
  public FilterTimestampColGreaterEqualLongColumn(int colNum1, int colNum2) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
  }

  public FilterTimestampColGreaterEqualLongColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (n != 0 && inputColVector1.isRepeating && inputColVector2.isRepeating) {
      ...
      if (!(inputColVector1.getTimestampAsLong(0) >= vector2[0])) {
        batch.size = 0;
      }
      ...
      if (batch.selectedInUse) {
        ...
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          if (inputColVector1.getTimestampAsLong(i) >= value2) {
            sel[newSize++] = i;
          }
        }
        batch.size = newSize;
      ...
      for(int j = 0; j != n; j++) {
        int i = sel[j];
        if (inputColVector1.getTimestampAsLong(i) >= vector2[i]) {
          sel[newSize++] = i;
        }
      }
      batch.size = newSize;

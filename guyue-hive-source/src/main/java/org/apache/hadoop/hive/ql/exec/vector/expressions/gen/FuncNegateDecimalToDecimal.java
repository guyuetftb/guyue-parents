...
public class FuncNegateDecimalToDecimal extends VectorExpression {
  private static final long serialVersionUID = 1L;

  private int colNum;
  private int outputColumn;

  public FuncNegateDecimalToDecimal(int colNum, int outputColumn) {
    this();
    this.colNum = colNum;
    this.outputColumn = outputColumn;
  }
  
  public FuncNegateDecimalToDecimal() {
    super();
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      DecimalUtil.negate(0, vector[0], outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          ...
          DecimalUtil.negate(i, vector[i], outputColVector);
        }
      } else {

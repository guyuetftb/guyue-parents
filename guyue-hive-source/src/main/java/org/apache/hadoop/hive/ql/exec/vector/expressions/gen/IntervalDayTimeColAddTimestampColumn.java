 ...
 */
public class IntervalDayTimeColAddTimestampColumn extends VectorExpression {
  ...
  public IntervalDayTimeColAddTimestampColumn(int colNum1, int colNum2, int outputColumn) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
    this.outputColumn = outputColumn;
  }

  public IntervalDayTimeColAddTimestampColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) {
      dtm.add(
          inputColVector1.asScratchIntervalDayTime(0), inputColVector2.asScratchTimestamp(0), outputColVector.getScratchTimestamp());
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.add(
              value1, inputColVector2.asScratchTimestamp(i), outputColVector.getScratchTimestamp());
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.add(
              inputColVector1.asScratchIntervalDayTime(i), value2, outputColVector.getScratchTimestamp());
          outputColVector.setFromScratchTimestamp(i);
        }
      } else {
        for(int i = 0; i != n; i++) {
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.add(
              inputColVector1.asScratchIntervalDayTime(i), inputColVector2.asScratchTimestamp(i), outputColVector.getScratchTimestamp());
          outputColVector.setFromScratchTimestamp(i);
        }
      } else {
        for(int i = 0; i != n; i++) {

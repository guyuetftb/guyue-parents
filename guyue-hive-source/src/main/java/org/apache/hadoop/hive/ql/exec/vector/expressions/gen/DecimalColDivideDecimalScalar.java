 ...
 */
public class DecimalColDivideDecimalScalar extends VectorExpression {
  ...
  public DecimalColDivideDecimalScalar(int colNum, HiveDecimal value, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
  ...
  public DecimalColDivideDecimalScalar() {
    this.outputType = "decimal";
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (value.compareTo(HiveDecimal.ZERO) == 0) {
      ...
      DecimalUtil.divideChecked(0, vector[0], value, outputColVector);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.divideChecked(i, vector[i], value, outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.divideChecked(i, vector[i], value, outputColVector);

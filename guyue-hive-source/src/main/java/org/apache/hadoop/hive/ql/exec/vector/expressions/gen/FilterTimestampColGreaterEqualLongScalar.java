 ...
 */
public class FilterTimestampColGreaterEqualLongScalar extends VectorExpression {
  ...
  public FilterTimestampColGreaterEqualLongScalar(int colNum, long value) {
    this.colNum = colNum;
    this.value = value;
  }

  public FilterTimestampColGreaterEqualLongScalar() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.noNulls) {
      if (inputColVector.isRepeating) {
        ...
        if (!(inputColVector.getTimestampAsLong(0) >= value)) {
        ...
        for(int j=0; j != n; j++) {
          int i = sel[j];
          if (inputColVector.getTimestampAsLong(i) >= value) {
            sel[newSize++] = i;
          }
        }
        batch.size = newSize;

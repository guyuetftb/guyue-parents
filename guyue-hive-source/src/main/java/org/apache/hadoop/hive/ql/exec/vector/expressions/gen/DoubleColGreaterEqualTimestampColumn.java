 ...
 */
public class DoubleColGreaterEqualTimestampColumn extends VectorExpression {
  ...
  public DoubleColGreaterEqualTimestampColumn(int colNum1, int colNum2, int outputColumn) {
    this.colNum1 = colNum1;
    this.colNum2 = colNum2;
    this.outputColumn = outputColumn;
  }

  public DoubleColGreaterEqualTimestampColumn() {
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) {
      outputVector[0] = vector1[0] >= inputColVector2.getDouble(0) ? 1 : 0;
    } else if (inputColVector1.isRepeating) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = vector1[0] >= inputColVector2.getDouble(i) ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = vector1[0] >= inputColVector2.getDouble(i) ? 1 : 0;
      ...
      double value2 = inputColVector2.getDouble(0);
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = vector1[i] >= inputColVector2.getDouble(i) ? 1 : 0;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = vector1[i] >= inputColVector2.getDouble(i) ? 1 : 0;

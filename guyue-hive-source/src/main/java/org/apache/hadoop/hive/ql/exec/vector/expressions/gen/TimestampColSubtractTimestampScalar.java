 ...
 */
public class TimestampColSubtractTimestampScalar extends VectorExpression {
  ...
  public TimestampColSubtractTimestampScalar(int colNum, Timestamp value, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating) {
      dtm.subtract(
          inputColVector1.asScratchTimestamp(0), value, outputColVector.getScratchIntervalDayTime());
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          dtm.subtract(
              inputColVector1.asScratchTimestamp(i), value, outputColVector.getScratchIntervalDayTime());

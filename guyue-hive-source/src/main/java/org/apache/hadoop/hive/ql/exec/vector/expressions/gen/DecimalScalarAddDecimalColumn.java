 ...
 */
public class DecimalScalarAddDecimalColumn extends VectorExpression {
  ...
  public DecimalScalarAddDecimalColumn(HiveDecimal value, int colNum, int outputColumn) {
    this.colNum = colNum;
    this.value = value;
  ...
  public DecimalScalarAddDecimalColumn() {
    this.outputType = "decimal";
  }
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      ...
      DecimalUtil.addChecked(0, value, vector[0], outputColVector);
    } else if (inputColVector.noNulls) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          DecimalUtil.addChecked(i, value, vector[i], outputColVector);
        }
      } else {
        for(int i = 0; i != n; i++) {
          DecimalUtil.addChecked(i, value, vector[i], outputColVector);

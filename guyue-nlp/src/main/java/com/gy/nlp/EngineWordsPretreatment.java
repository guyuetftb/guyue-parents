...
public class EngineWordsPretreatment extends Configured implements Tool {

    private final static Logger logger = Logger.getLogger(EngineWordsPretreatment.class);
        ...
        public void map(Object keyObj, Text line, Context context) {
            ...
            for (String w : wordArr) {
                ...
                if (matcher.find()) {
                    try {
                        wordKey.set(w);
                        context.getCounter(StatisticEnum.STATIC_WORD_YES).increment(1);
                    ...
                    context.getCounter(StatisticEnum.STATIC_WORD_NO).increment(1);
                }
            }
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, ClassUtils.getShortClassName(EngineWordsPretreatment.class) + "[" + countryCode + "]");
        if (job == null) {
        ...
        // set mapper
        job.setJarByClass(EngineWordsPretreatment.class);
    ...
    public static int executeTask(String args[]) {
        ...
        int code = 0;
        try {
            code = ToolRunner.run(new EngineWordsPretreatment(), args);
        } catch (Exception e) {

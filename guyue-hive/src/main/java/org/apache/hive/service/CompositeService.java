...
public class CompositeService extends AbstractService {
    ...
    public CompositeService(String name) {
        super(name);
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 CompositeService(name)");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 CompositeService(name)");
    }
    ...
    protected synchronized void addService(Service service) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 addService(Service service) ");
        LOG.info(LOG_GY_PREFIX + " 1 \t addService(Service service) service = " + service.getClass().getName());
        serviceList.add(service);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 addService(Service service) ");
    }
    ...
    public synchronized void init(HiveConf hiveConf) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 init(hiveConf)");
        for (Service service : serviceList) {
            LOG.info(LOG_GY_PREFIX + " \t 0 init(hiveConf) service = " + service);
            service.init(hiveConf);
        }
        super.init(hiveConf);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 init(hiveConf)");
    }
    ...
    public synchronized void start() {
        ...
        try {
            for (int n = serviceList.size(); i < n; i++) {
                ...
                LOG.info(LOG_GY_PREFIX + " \t 0 start() service = " + service.getClass().getName());
                LOG.info(LOG_GY_PREFIX + " \t 1 start() service.start_time = " + service.getStartTime());
                service.start();
            }
            super.start();
        } catch (Throwable e) {

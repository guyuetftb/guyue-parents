...
public class CLIService extends CompositeService implements ICLIService {
    ...
    public CLIService(HiveServer2 hiveServer2) {
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " CLIService(hiveServer2)");
        LOG.info(LOG_GY_PREFIX + " 1 \t CLIService(hiveServer2)");
        this.hiveServer2 = hiveServer2;
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " CLIService(hiveServer2)");
    }
    ...
    public SessionHandle openSession(TProtocolVersion protocol,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 openSession(protocol, username, password, configurationMap) 1111 ");
        SessionHandle sessionHandle = sessionManager.openSession(protocol, username, password, null, configuration, false, null);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 openSession(protocol, username, password, configurationMap) 1111 ");
     ...
     */
    @Deprecated
    public SessionHandle openSessionWithImpersonation(TProtocolVersion protocol,
                                                      String username,
                                                      String password,
                                                      Map<String, String> configuration,
                                                      String delegationToken)
            throws HiveSQLException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 openSessionWithImpersonation(protocol, username, password, configuration, delegationToken) ");
        SessionHandle sessionHandle = sessionManager.openSession(protocol, username, password, null, configuration,
                                                                 true, delegationToken);
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": openSessionWithImpersonation(protocol, username, password, configuration, delegationToken)");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 openSessionWithImpersonation(protocol, username, password, configuration, delegationToken) ");
        return sessionHandle;
    }
    ...
    public SessionHandle openSession(TProtocolVersion protocol,
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": openSession(111)");
        return sessionHandle;
    }
    ...
    public SessionHandle openSessionWithImpersonation(TProtocolVersion protocol,
                                                      ...
                                                      String delegationToken)
            throws HiveSQLException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 openSessionWithImpersonation(protocol, username, password, ipAddress, configuration, delegationToken) ");
        SessionHandle sessionHandle = sessionManager.openSession(protocol, username, password, ipAddress, configuration,
                                                                 true, delegationToken);
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + " : openSessionWithImpersonation(protocol, username, password, ipAddress, configuration, delegationToken) ");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 openSessionWithImpersonation(protocol, username, password, ipAddress, configuration, delegationToken) ");
        return sessionHandle;
    }
    ...
    public SessionHandle openSession(String username,
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": openSession(222)");
        return sessionHandle;
    }
    ...
    public void createSessionWithSessionHandle(SessionHandle sessionHandle,
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": createSessionWithSessionHandle(333)");
    }
    ...
    public SessionHandle openSessionWithImpersonation(String username,
        ...
        SessionHandle sessionHandle = sessionManager.openSession(SERVER_VERSION, username, password, null, configuration,
                                                                 true, delegationToken);
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": openSession(444)");
        return sessionHandle;
    }
    ...
    public void closeSession(SessionHandle sessionHandle)
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": closeSession()");
    }
    ...
    public OperationHandle executeStatement(SessionHandle sessionHandle,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 executeStatement() ------->> 1111");
        HiveSession session = sessionManager.getSession(sessionHandle);
        LOG.info(LOG_GY_PREFIX + " \t 1 executeStatement(1111) session = " + session.getClass().getSimpleName());
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 " + sessionHandle + ": executeStatement(1111) statement = " + statement);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 executeStatement() ------->> 1111");
        return opHandle;
    }
    ...
    public OperationHandle executeStatement(SessionHandle sessionHandle,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 executeStatement() ------->> 2222");
        HiveSession session = sessionManager.getSession(sessionHandle);
        LOG.info(LOG_GY_PREFIX + " \t 1 executeStatement(2222) session = " + session.getClass().getSimpleName());
        // need to reset the monitor, as operation handle is not available down stream, Ideally the
        // monitor should be associated with the operation handle.
        session.getSessionState().updateProgressMonitor(null);
        ...
        /**
         * TODO lipeng
         * CLIService.execumentStatement -> HiveSessionImpl.executeStatement
         */
        OperationHandle opHandle = session.executeStatement(statement, confOverlay, queryTimeout);
        LOG.info(LOG_GY_PREFIX + " \t 2 " + sessionHandle + ": executeStatement(2222) statement = " + statement);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 executeStatement() ------->> 2222");
        return opHandle;
    }
    ...
    public OperationHandle executeStatementAsync(SessionHandle sessionHandle,
                                                 String statement,
                                                 Map<String, String> confOverlay) throws HiveSQLException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 executeStatementAsync() =======>> 3333");
        HiveSession session = sessionManager.getSession(sessionHandle);
        LOG.info(LOG_GY_PREFIX + " \t 1 executeStatementAsync(4444) session = " + session.getClass().getSimpleName());
        // need to reset the monitor, as operation handle is not available down stream, Ideally the
        // monitor should be associated with the operation handle.
        session.getSessionState().updateProgressMonitor(null);
        OperationHandle opHandle = session.executeStatementAsync(statement, confOverlay);
        LOG.info(LOG_GY_PREFIX + " \t 2 " + sessionHandle + ": executeStatementAsync(3333) statement = " + statement);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 executeStatementAsync()  =======>> 3333");
        return opHandle;
    }
    ...
    public OperationHandle executeStatementAsync(SessionHandle sessionHandle,
                                                 ...
                                                 Map<String, String> confOverlay,
                                                 long queryTimeout) throws HiveSQLException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 executeStatementAsync()  =======>> 4444");
        HiveSession session = sessionManager.getSession(sessionHandle);
        LOG.info(LOG_GY_PREFIX + " \t 1 executeStatementAsync(4444) session = " + session.getClass().getSimpleName());
        // need to reset the monitor, as operation handle is not available down stream, Ideally the
        // monitor should be associated with the operation handle.
        ...
        LOG.info(LOG_GY_PREFIX + " \t 2 " + sessionHandle + ": executeStatementAsync(4444) statement = " + statement);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 executeStatementAsync() =======>> 4444");
        return opHandle;
    }
    ...
    public RowSet fetchResults(OperationHandle opHandle)
            throws HiveSQLException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " fetchResults(OperationHandle opHandle) ");
        RowSet rs = fetchResults(opHandle, Operation.DEFAULT_FETCH_ORIENTATION,
                                 defaultFetchRows, FetchType.QUERY_OUTPUT);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " fetchResults(OperationHandle opHandle) ");
        return rs;
    }
    ...
    public RowSet fetchResults(OperationHandle opHandle,
        ...
        /**
         * TODO lipeng
         * ThriftCLIService.FetchResults -> CLIService.fetchResults -> HiveSessionImpl.fetchResults
         */
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " fetchResults(OperationHandle opHandle) ");
        ...
        LOG.info(LOG_GY_PREFIX + " \t opHandle = " + opHandle + ": fetchResults()");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " fetchResults(OperationHandle opHandle) ");
        return rowSet;
    }

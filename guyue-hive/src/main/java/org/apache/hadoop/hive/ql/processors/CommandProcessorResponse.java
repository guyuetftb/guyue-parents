...
public class CommandProcessorResponse {
	...
	public CommandProcessorResponse(int responseCode, ErrorMsg canonicalErrMsg, Throwable t, String... msgArgs) {
		this(responseCode,
				canonicalErrMsg.format(msgArgs),
				canonicalErrMsg.getSQLState(),
				null,
				t,
				canonicalErrMsg.getErrorCode(),
				null);
	}
	...
	public CommandProcessorResponse(int responseCode, String errorMessage, String SQLState, Schema schema,
									Throwable exception, int hiveErrorCode, List<String> consoleMessages) {
	...
	public String toString() {
		return "(" + responseCode + "," + errorMessage + "," + (hiveErrorCode > 0 ? hiveErrorCode + "," : "") + SQLState
			   + (resSchema == null ? "" : ",") + (exception == null ? "" : exception.getMessage()) + ")";
	}
}

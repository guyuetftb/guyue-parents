 ...
 * jvm. <br/>
 * QueryPlan可以序列化到磁盘上，以至于我们可以在未来 重启 或 恢复对它的改进，或者在内部/外部JVM中打开它，
 */
public class QueryPlan implements Serializable {
	...
	private final Map<String, Map<String, Long>> counters = new ConcurrentHashMap<String, Map<String, Long>>();
	private final Set<String>                    done     = Collections.newSetFromMap(new ConcurrentHashMap<String, Boolean>());
	private final Set<String>                    started  = Collections.newSetFromMap(new ConcurrentHashMap<String, Boolean>());
	...
	public QueryPlan(String queryString, BaseSemanticAnalyzer sem, Long startTime, String queryId,
		...
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan(String, BaseSemanticAnalyzer, Long, String, HiveOperation, Schema) ");
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan queryString = " + queryString);
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan sem = " + sem.getClass().getName());
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan startTime = " + startTime);
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan queryId = " + queryId);
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan HiveOperation = " + operation.toString() + ", operation-name = "
				 + operation.getOperationName());
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan resultSchema = " + resultSchema.toString());
		...
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan queryProperties = " + queryProperties);
		queryStartTime = startTime;
		this.operation = operation;

		LOG.info(LOG_GY_PREFIX + " \t QueryPlan operation = " + operation);
		this.autoCommitValue = sem.getAutoCommitValue();

		LOG.info(LOG_GY_PREFIX + " \t QueryPlan resultSchema = " + resultSchema);
	...
	public static String makeQueryId() {
		...
		return userid + "_" + String.format("%1$4d%2$02d%3$02d%4$02d%5$02d%6$02d",
				gc.get(Calendar.YEAR),
				gc.get(Calendar.MONTH) + 1,
				gc.get(Calendar.DAY_OF_MONTH),
				gc.get(Calendar.HOUR_OF_DAY),
				gc.get(Calendar.MINUTE),
				gc.get(Calendar.SECOND)) + "_" + UUID.randomUUID().toString();
	}
	 ...
	 * @param topOps the set of top operators from which the operator graph for the
	 *               task is hanging
	 */
	...
	@SuppressWarnings("rawtypes") private String getJSONList(List list) {
		if (list == null) {
			return "null";
		}
	...
	@SuppressWarnings("rawtypes") public String getJSONMap(Map map) {
		if (map == null) {
			return "null";
		}
		StringBuilder sb = new StringBuilder();
	...
	public void setStarted() {
		LOG.info(LOG_GY_PREFIX + " \t QueryPlan().setStarted() ");
		started.add(queryId);
	}

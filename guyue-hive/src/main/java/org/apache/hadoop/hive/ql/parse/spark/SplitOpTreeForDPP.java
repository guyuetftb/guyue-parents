...
public class SplitOpTreeForDPP implements NodeProcessor {
	@Override public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procCtx, Object... nodeOutputs)
		...
		for (Operator<?> childOp : filterOp.getChildOperators()) {
			if (childOp instanceof ReduceSinkOperator && childOp.getChildOperators()
		...
		Preconditions.checkArgument(sinkSet.size() == 1,
				"AssertionError: expected to only contain one SparkPartitionPruningSinkOperator," + " but found "
				+ sinkSet.size());
		SparkPartitionPruningSinkOperator clonedPruningSinkOp = (SparkPartitionPruningSinkOperator) sinkSet.iterator()
				.next();

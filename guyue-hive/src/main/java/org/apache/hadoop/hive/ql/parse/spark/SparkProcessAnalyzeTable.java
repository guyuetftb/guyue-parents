...
public class SparkProcessAnalyzeTable implements NodeProcessor {
	...
	public Object process(Node nd, Stack<Node> stack, NodeProcessorCtx procContext, Object... nodeOutputs)
		...
		@SuppressWarnings("rawtypes") Class<? extends InputFormat> inputFormat = tableScan.getConf()
				.getTableMetadata()
				.getInputFormatClass();

		if (parseContext.getQueryProperties().isAnalyzeCommand()) {
			Preconditions.checkArgument(
					tableScan.getChildOperators() == null || tableScan.getChildOperators().size() == 0,
					"AssertionError: expected tableScan.getChildOperators() to be null, "
					+ "or tableScan.getChildOperators().size() to be 0");

			String alias = null; for (String a : parseContext.getTopOps().keySet()) {
			...
			if (inputFormat.equals(OrcInputFormat.class) && (noScan || partialScan)) {
				...
				snjTask.setParentTasks(null); context.rootTasks.remove(context.currentTask);
				context.rootTasks.add(snjTask); return true;
			} else {
				...
				if (parseContext.getQueryProperties().isNoScanAnalyzeCommand()) {
					statsTask.setParentTasks(null); statsWork.setNoScanAnalyzeCommand(true);
					context.rootTasks.remove(context.currentTask); context.rootTasks.add(statsTask);
				}
				...
				PrunedPartitionList partitions = null; if (confirmedPartns.size() > 0) {
				...
				MapWork w = utils.createMapWork(context, tableScan, sparkWork, partitions); w.setGatheringStats(true);
				return true;
			}
		}
	 ...
	 */
	private void handlePartialScanCommand(TableScanOperator tableScan, ParseContext parseContext, StatsWork statsWork,
										  GenSparkProcContext context, Task<StatsWork> statsTask)
			throws SemanticException {
		...
		// scan work
		PartialScanWork scanWork = new PartialScanWork(inputPaths); scanWork.setMapperCannotSpanPartns(true);
		...
		@SuppressWarnings("unchecked") Task<PartialScanWork> partialScanTask = TaskFactory.get(scanWork,
		...
		partialScanTask.setWork(scanWork); statsWork.setSourceTask(partialScanTask);

		// task dependency
		context.rootTasks.remove(context.currentTask); context.rootTasks.add(partialScanTask);

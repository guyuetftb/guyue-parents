...
public class TezTask extends Task<TezWork> {
    ...
    public TezTask() {
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 1 TezTask() ");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 1 TezTask() ");
    }

    public TezTask(DagUtils utils) {
        super();
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 2 TezTask(DagUtils)");
        this.utils = utils;
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 2 TezTask(DagUtils)");
    }
    ...
    public int execute(DriverContext driverContext) {
        perfLogger.PerfLogBegin(CLASS_NAME, "execute");
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " execute(DriverContext) ");
        int rc = 1;
        ...
        try {
            ...
            try {
                ...
                LOG.info(LOG_GY_PREFIX + " \t  execute(DriverContext) job-name = " + jobConf.getJobName());
                ...
                Map<String, LocalResource> inputOutputLocalResources = getExtraLocalResources(jobConf,
                                                                                              scratchDir,
                                                                                              inputOutputJars);
                ...
                try {
                    LOG.info(LOG_GY_PREFIX + " \t  execute(DriverContext) tezClient = " + ss.getTezSession()
                            .getSession());
                    LOG.info(
                            LOG_GY_PREFIX + " \t  execute(DriverContext) Gy-Yue-applicationId = " + ss.getTezSession()
                                    .getSession()
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " execute(DagUtils)");
        return rc;
    }
     ...
     */
    Map<String, LocalResource> getExtraLocalResources(JobConf jobConf,
                                                      Path scratchDir,
     ...
     */
    void addExtraResourcesToDag(TezSessionState session,
                                DAG dag,
                                String[] inputOutputJars,
    ...
    void checkOutputSpec(BaseWork work,
                         JobConf jc) throws IOException {
    ...
    DAG build(JobConf conf,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN
                         + " build(JobConf,TezWork,Path,LocalResource,List<LocalResource>,Context) ");
        ...
        LOG.info(
                LOG_GY_PREFIX + " \t build(JobConf,TezWork,Path,LocalResource,List<LocalResource>,Context) Dag-Name = "
                        + dagName);
        ...
        LOG.info(
                LOG_GY_PREFIX + " \t build(JobConf,TezWork,Path,LocalResource,List<LocalResource>,Context) dag-Info = "
                        + dagInfo);
        ...
        for (BaseWork w : ws) {
            ...
            if (w instanceof UnionWork) {
                ...
                for (BaseWork v : children) {
                    ...
                    GroupInputEdge e = utils.createEdge(group,
                                                        parentConf,
                                                        workToVertex.get(v),
                                                        work.getEdgeProperty(w, v),
                ...
                Vertex wx = utils.createVertex(wxConf,
                                               w,
                                               scratchDir,
                                               appJarLr,
                                               additionalLr,
    ...
    private static void setAccessControlsForCurrentUser(DAG dag,
                                                        String queryId,
                                                        Configuration conf)
            throws IOException {
        ...
        String viewStr = Utilities.getAclStringWithHiveModification(conf,
                                                                    TezConfiguration.TEZ_AM_VIEW_ACLS,
                                                                    addHs2User,
                                                                    user,
                                                                    loginUser);
        String modifyStr = Utilities.getAclStringWithHiveModification(conf,
                                                                      TezConfiguration.TEZ_AM_MODIFY_ACLS,
                                                                      addHs2User,
                                                                      user,
                                                                      loginUser);
    ...
    DAGClient submit(JobConf conf,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN
                         + " submit(JobConf,DAG,Path,LocalResource,TezSessionState,List<LocalResource>,String[],Map<String, LocalResource>) ");

...
public class SetProcessor implements CommandProcessor {
    ...
    public static CommandProcessorResponse setVariable(
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " setVariable(String,String) ");
        SessionState ss = SessionState.get();
        ...
        if (varname.startsWith(ENV_PREFIX)) {
            LOG.info(LOG_GY_PREFIX + " \t 0 setVariable(String,String) ");
            ss.err.println("env:* variables can not be set.");
            return new CommandProcessorResponse(1); // Should we propagate the error message properly?
        } else if (varname.startsWith(SYSTEM_PREFIX)) {
            LOG.info(LOG_GY_PREFIX + " \t 1 setVariable(String,String) ");
            ...
            LOG.info(LOG_GY_PREFIX + " \t 2 setVariable(String,String) ");
            String propName = varname.substring(HIVECONF_PREFIX.length());
            nonErrorMessage = setConf(varname, propName, varvalue, false);
        } else if (varname.startsWith(HIVEVAR_PREFIX)) {
            LOG.info(LOG_GY_PREFIX + " \t 3 setVariable(String,String) ");
            ...
            LOG.info(LOG_GY_PREFIX + " \t 4 setVariable(String,String) ");
            ...
            LOG.info(LOG_GY_PREFIX + " \t 5 setVariable(String,String) ");
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 1 run(Stringd) ");
    ...
    public CommandProcessorResponse run(String command) {
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " run(Stringd) ");
        ...
        if (nwcmd.equals("-v")) {
            LOG.info(LOG_GY_PREFIX + " \t 1 run(Stringd) command = " + nwcmd);
            ...
            CommandProcessorResponse cpr = createProcessorSuccessResponse();
            LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 0 run(Stringd) ");
            return cpr;
        }
        ...
        if (nwcmd.contains("=")) {
            ...
            LOG.info(LOG_GY_PREFIX + " \t 2 run(Stringd) command = " + command);
            CommandProcessorResponse cpr = executeSetVariable(part[0], part[1]);
            LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 1 run(Stringd) ");
            return cpr;
        }
        try {
            CommandProcessorResponse cpr = getVariable(nwcmd);
            LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 2 run(Stringd) ");
            return cpr;
        } catch (Exception e) {
            return new CommandProcessorResponse(1, e.getMessage(), "42000", e);
        }

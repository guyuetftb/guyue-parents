...
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
    ...
    private final Logger LOG = LoggerFactory.getLogger(SemanticAnalyzer.class.getName());
    ...
    public ParseContext getParseContext() {
        ...
        return new ParseContext(queryState,
                                opToPartPruner,
                                opToPartList,
                                topOps,
    ...
    public void doPhase1QBExpr(ASTNode ast,
                               ...
                               String id,
                               String alias,
                               boolean insideView) throws SemanticException {
        ...
        // setop
            ...
            switch (ast.getToken().getType()) {
                ...
                case HiveParser.TOK_EXCEPTDISTINCT:
                    ...
                    break;
                default:
                    throw new SemanticException(ErrorMsg.UNSUPPORTED_SET_OPERATOR.getMsg(
...
//        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " doPhase1GetAggregationsFromSelect(ASTNode,QB,String) ");
...
//                LOG.info(LOG_GY_PREFIX + " \t doPhase1GetAggregationsFromSelect(ASTNode,QB,String) function = "
//                                 + function.getText() + ", name = " + function.getName() + ", text = " + function.getText()
//                                 + ", token=" + function.getToken().toString());
            }
...
//        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " doPhase1GetAggregationsFromSelect(ASTNode,QB,String) ");
        return aggregationTrees;
    }

    private void doPhase1GetColumnAliasesFromSelect(ASTNode selectExpr,
                                                    QBParseInfo qbp) {
//        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " doPhase1GetColumnAliasesFromSelect(ASTNode,QBParseInfo) ");
...
//                LOG.info(LOG_GY_PREFIX + " \t doPhase1GetColumnAliasesFromSelect(ASTNode,QBParseInfo) columnAlias = "
//                                 + columnAlias + ", type-text = " + selExpr.getChild(0).getText());
            }
        }
//        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " doPhase1GetColumnAliasesFromSelect(ASTNode,QBParseInfo) ");
    }
     ...
     * @param aggregations   the key to the HashTable is the toStringTree() representation of
    ...
    private void doPhase1GetAllAggregations(ASTNode expressionTree,
                                            ...
                                            List<ASTNode> wdwFns) throws SemanticException {

//        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN
//                         + " doPhase1GetAllAggregations(ASTNode,HashMap<String, ASTNode>,List<ASTNode>) ");
...
//                                 + " \t doPhase1GetAllAggregations(ASTNode,HashMap<String, ASTNode>,List<ASTNode>) funcationName = "
//                                 + functionName);
...
//        LOG.info(LOG_GY_PREFIX + LOG_GY_END
//                         + " doPhase1GetAllAggregations(ASTNode,HashMap<String, ASTNode>,List<ASTNode>) ");
    }

    private List<ASTNode> doPhase1GetDistinctFuncExprs(HashMap<String, ASTNode> aggregationTrees) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN
                         + " doPhase1GetDistinctFuncExprs(HashMap<String, ASTNode> aggregationTrees) ");
        ...
        for (Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            if (value.getToken().getType() == HiveParser.TOK_FUNCTIONDI) {
                exprs.add(value);
                LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN
                                 + " \t\t doPhase1GetDistinctFuncExprs(HashMap<String, ASTNode> aggregationTrees) text = "
                                 + value.getText() + ", string-tree = " + value.toStringTree());
            }
        }
        LOG.info(LOG_GY_PREFIX
                         + " 1 doPhase1GetDistinctFuncExprs(ASTNode,HashMap<String, ASTNode>,List<ASTNode>) NumSelDi = "
                         + qb.getNumSelDi());
        LOG.info(LOG_GY_PREFIX + LOG_GY_END
                         + " doPhase1GetDistinctFuncExprs(ASTNode,HashMap<String, ASTNode>,List<ASTNode>) ");
        return exprs;
    }
     ...
     */
    private String processTable(QB qb,
                                ASTNode tabref) throws SemanticException {

        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " processTable(QB,ASTNode) ");
        ...
        LOG.info(LOG_GY_PREFIX + " 1 processTable(QB,ASTNode) tabIdName = " + tabIdName);

        String alias = findSimpleTableName(tabref, aliasIndex);
        LOG.info(LOG_GY_PREFIX + " 2 processTable(QB,ASTNode) alias = " + alias);
        ...
        if (tsampleIndex >= 0) {
            ...
            if (sampleCols.size() > 2) {
                throw new SemanticException(generateErrorMessage((ASTNode) tabref.getChild(0),
                                                                 ErrorMsg.SAMPLE_RESTRICTION.getMsg()));
            }
            TableSample tabSample = new TableSample(unescapeIdentifier(sampleClause.getChild(0).getText()),
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " processTable(QB,ASTNode) ");
        return alias;
    }
     ...
     */
    private static void writeAsText(String text,
     ...
     */
    private ASTNode genValuesTempTable(ASTNode originalFrom,
        ...
        assert valuesTable.getToken().getType() == HiveParser.TOK_VALUES_TABLE :
                "Expected second child of TOK_VIRTUAL_TABLE to be TOK_VALUE_TABLE but was " + valuesTable.getName();
    ...
    private void assertCombineInputFormat(Tree numerator,
                                          String message) throws SemanticException {
        String inputFormat = conf.getVar(HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("tez") ? HiveConf.getVar(conf,
                                                                                                                  HiveConf.ConfVars.HIVETEZINPUTFORMAT) : HiveConf.getVar(conf, HiveConf.ConfVars.HIVEINPUTFORMAT);
    ...
    private String processSubQuery(QB qb,
                                   ASTNode subq) throws SemanticException {
     ...
     */
    private void processCTE(QB qb,
                            ASTNode ctes) throws SemanticException {
     ...
     *
     */
    private CTEClause findCTEFromName(QB qb,
                                      String cteName) {
     ...
     */
    private void addCTEAsSubQuery(QB qb,
                                  String cteName,
                                  String cteAlias) throws SemanticException {
    ...
    class CTEClause {
        ...
        void asExecutionOrder(Set<CTEClause> visited,
                              List<CTEClause> execution) {
    ...
    Table materializeCTE(String cteName,
                         CTEClause cte) throws HiveException {
    ...
    static boolean isJoinToken(ASTNode node) {
        return (node.getToken().getType() == HiveParser.TOK_JOIN) || (node.getToken().getType()
                == HiveParser.TOK_CROSSJOIN) || isOuterJoinToken(
                node) || (node.getToken().getType() == HiveParser.TOK_LEFTSEMIJOIN) || (node.getToken().getType()
                == HiveParser.TOK_UNIQUEJOIN);

    }

    static private boolean isOuterJoinToken(ASTNode node) {
        return (node.getToken().getType() == HiveParser.TOK_LEFTOUTERJOIN) || (node.getToken().getType()
     ...
     */
    @SuppressWarnings("nls")
    private void processJoin(QB qb,
                             ASTNode join) throws SemanticException {
    ...
    private String processLateralView(QB qb,
     ...
     * appropriate mapping in aliasToTabs, aliasToSubq <br/>
     * 获得所有表、子查询的别名，并将 相应的 数据保存在 aliasToTabs 和 aliasToSubq中. <br/>
     * 2. Gets the location of the
     * destination and names the clause "inclause" + i <br/>
     * 获得目标位置并对各个子句进行  "inclause" + i 的命名.
     * 3. Creates a map from a
     * string representation of an aggregation tree to the actual aggregation AST <br/>
     * 创建一个 从 字符聚合串聚合Tree 至 实际 AST聚合 之间的 映射. <br/>
     * 4. Creates a mapping from the clause name to the select expression AST in
     * destToSelExpr <br/>
     * 创建一个 从子句名称 至 Select AST 表达式之间的映射. <br/>
     * 5. Creates a mapping from a table alias to the lateral view
     * AST's in aliasToLateralViews <br/>
     * 创建 从 表别名 到 横向AST视图 的映射
     *
     * @param ast
     * @param qb
     * @param ctx_1
    ...
    public boolean doPhase1(ASTNode ast,
        ...
        /**
         * doPhase1执行完毕之后得到QB，QB里边的只是一些关键字还有一些表的名字，
         * 但是和hdfs的文件路径对应不起来，所以需要metaData映射关系，
         * 之后在SemanticAnalyzer中调用了 getMetaData.
         */

//        LOG.info(LOG_GY_PREFIX + " \t 1 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) ast-text = " + ast.getText());
//        LOG.info(LOG_GY_PREFIX + " \t 2 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) plannerCtx = " + plannerCtx);
        boolean phase1Result = true;
        QBParseInfo qbp = qb.getParseInfo();

//        LOG.info(LOG_GY_PREFIX + " \t 3 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) QBParseInfo.alias = "
//                         + qbp.getAlias());
...
//                    LOG.info(
//                            LOG_GY_PREFIX + " \t \t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
//                                    + ast.getText());
//                    LOG.info(LOG_GY_PREFIX
//                                     + " \t \t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) child-count = " + ast
//                            .getChildCount());
//                    LOG.info(LOG_GY_PREFIX
//                                     + " \t \t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) ast-tree="
//                                     + ast.toStringTree());
//                    LOG.info(
//                            LOG_GY_PREFIX + " \t \t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
//                                    + ast.getText() + ", NumSels = " + qb.getNumSels());
//                    LOG.info(
//                            LOG_GY_PREFIX + " \t \t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
//                                    + ast.getText() + ", NumSelDi = " + qb.getNumSelDi());
...
//                        LOG.info(LOG_GY_PREFIX
//                                         + " \t \t\t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text=" + text
//                                         + ", child-count = " + childCount + ", string-tree=" + stringTree);
                        for (int i = 0; i < childCount; i++) {
                            ASTNode childNode = (ASTNode) astNode.getChild(i);
//                            LOG.info(LOG_GY_PREFIX
//                                             + " \t \t\t\t- 4-TOK_SELECT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text="
//                                             + childNode.getText() + ", child-count = " + childNode.getChildCount()
//                                             + ", string-tree=" + childNode.toStringTree());
//                            LOG.info(LOG_GY_PREFIX + " \t \t\t\t ");
                        }
                    }
                ...
                case HiveParser.TOK_WHERE:
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t \t- 5-TOK_WHERE doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_INSERT_INTO:
                    ...
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 6-2-TOK_INSERT_INTO doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = " + ast
                            .getText() + ", ast-tree=" + ast.toStringTree());
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 6-1-TOK_INSERT_INTO doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) currentDatabase = "
                                     + currentDatabase + ", tab_name=" + tab_name);
                ...
                case HiveParser.TOK_DESTINATION:
                    ...
                    ctx_1.nextNum++;
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 7-TOK_DESTINATION doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_FROM:
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t \t- 8-TOK_FROM doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                    if (child_count != 1) {
                    ...
                    if (frm.getToken().getType() == HiveParser.TOK_TABREF) {
                        LOG.info(LOG_GY_PREFIX
                                         + " \t \t- 9-TOK_TABREF doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                         + ast.getText() + ", ast-tree=" + ast.toStringTree());
                        processTable(qb, frm);
                        ...
                        LOG.info(LOG_GY_PREFIX
                                         + " \t \t- 10-TOK_SUBQUERY doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = " + ast
                                .getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_CLUSTERBY:
                    ...
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 11-TOK_CLUSTERBY doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_DISTRIBUTEBY:
                    ...
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 12-TOK_DISTRIBUTEBY doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = " + ast
                            .getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_SORTBY:
                    ...
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 13-TOK_SORTBY doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_ORDERBY:
                    ...
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 14-TOK_ORDERBY doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_GROUPING_SETS:
                    ...
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 15-TOK_ORDERBY doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_HAVING:
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 16-TOK_HAVING doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.KW_WINDOW:
                    LOG.info(LOG_GY_PREFIX
                                     + " \t \t- 17-TOK_HAVING doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_LIMIT:
                    LOG.info(
                            LOG_GY_PREFIX + " \t \t- 18-TOK_LIMIT doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) text = "
                                    + ast.getText() + ", ast-tree=" + ast.toStringTree());
                ...
                case HiveParser.TOK_UNIONALL:
                    LOG.info(LOG_GY_PREFIX + " \t \t- 19 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) TOK_UNIONALL = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                    if (!qbp.getIsSubQ()) {
                ...
                case HiveParser.TOK_INSERT:
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t \t- 20 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) TOK_INSERT = "
                                     + ast.getText() + ", ast-tree=" + ast.toStringTree());
                    ...
                    if (destination.getChildCount() == 2 && tab.getChildCount() == 2
                        ...
                        for (int i = 0; i < childCount; i++) {
                            ...
                            LOG.info(LOG_GY_PREFIX
                                             + " \t \t- 20-1 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) partitionName = "
                                             + partitionName + ", partitionVal = " + partitionVal);
                            partition.put(partitionName, partitionVal);
                        }
                        // if it is a dynamic partition throw the exception
                        if (childCount != partition.size()) {
                            throw new SemanticException(ErrorMsg.INSERT_INTO_DYNAMICPARTITION_IFNOTEXISTS.getMsg(
                                    partition.toString()));
                        }
                        Table table = null;
                        try {
                            LOG.info(LOG_GY_PREFIX
                                             + " \t \t- 20-2 doPhase1(ASTNode,QB,Phase1Ctx,PlannerContext) tableName = "
                                             + tableName);
        ...
        if (!skipRecursion) {
            ...
            for (int child_pos = 0; child_pos < child_count && phase1Result; ++child_pos) {
                ASTNode childASTNode = (ASTNode) ast.getChild(child_pos);
                LOG.info(LOG_GY_PREFIX + " \t -ast-name=" + childASTNode.getText() + ", String-tree = "
                                 + childASTNode.toStringTree());
                // Recurse, 递归调用 (深度优先搜索的算法)
     ...
     */
    private void handleInsertStatementSpecPhase1(ASTNode ast,
                                                 QBParseInfo qbp,
                                                 Phase1Ctx ctx_1)
            throws SemanticException {
                ...
                if (ast.getChild(0) != null && ast.getChild(0).getType() == HiveParser.TOK_TAB) {
                    ...
                    if (tokPartSpec != null) {
                        for (Node n : tokPartSpec.getChildren()) {
                            ...
                            if (tokPartVal != null && tokPartVal.getType() == HiveParser.TOK_PARTVAL
                ...
                if (!targetColumns.isEmpty()) {
                    ...
                    throw new SemanticException(generateErrorMessage(tabColName,
                                                                     "'" + (targetColumns.size() == 1 ? targetColumns.iterator().next() : targetColumns)
                                                                             ...
                                                                             + " not found among regular columns of " + fullTableName
    ...
    private void gatherCTEReferences(QBExpr qbexpr,
                                     CTEClause parent) throws HiveException {
    ...
    private void gatherCTEReferences(QB qb,
                                     CTEClause current) throws HiveException {
            ...
            if (cte != null) {
                if (ctesExpanded.contains(cteName)) {
                    throw new SemanticException(
                            "Recursive cte " + cteName + " detected (cycle: " + StringUtils.join(ctesExpanded, " -> ")
                                    + " -> " + cteName + ").");
                }
                cte.reference++;
    ...
    public void getMetaData(QB qb,
                            boolean enableMaterialization) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + "getMetaData(QB,boolean) ");
        try {
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " getMetaData(QB,boolean) ");
    }

    private void getMetaData(QBExpr qbexpr,
                             ReadEntity parentInput) throws HiveException {
        if (qbexpr.getOpcode() == QBExpr.Opcode.NULLOP) {
    ...
    @SuppressWarnings("nls")
    private void getMetaData(QB qb,
                             ReadEntity parentInput) throws HiveException {
        /**
         * getMetaData又会递归的去取元数据（从mysql中），
         * 经过doPhase1、getMetaData得到一个完整的QB，
         * 接下来就是逻辑执行技术的生成
         */
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + "getMetaData(QB,ReadEntity) Get metadata for source tables.");
        if (null != qb.getParseInfo() && null != qb.getParseInfo().getQueryFrom()) {
            LOG.info(LOG_GY_PREFIX + " \t 1 getMetaData(QB,ReadEntity) query-from = " + qb.getParseInfo()
                    .getQueryFrom()
                    .toStringTree());
        }
        ...
        // 搜索检查所有的表，并填充相关的结构.
        // 我们必须具体化表别名 列表，因为我们可能在重写视频过程中修改它。
        List<String> tabAliases = new ArrayList<String>(qb.getTabAliases());
        LOG.info(LOG_GY_PREFIX + " \t 2 getMetaData(QB,ReadEntity) tabAliases = " + tabAliases.toString());

        // Keep track of view alias to view name and read entity
        // 保持对视图别名的跟踪，查看视图名称，读取他的实体.
        // For eg: for a query like 'select * from V3', where V3 -> V2, V2 -> V1, V1 -> T
        ...
        for (String alias : tabAliases) {
            ...
            if (null != tab) {
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab = " + tab);
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getTableType = " + tab.getTableType());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getPath = " + tab.getPath().toString());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getInputFormatClass = " + tab.getInputFormatClass().getName());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getOwner = " + tab.getOwner());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getDbName = " + tab.getDbName());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getCompleteName = " + tab.getCompleteName());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getCreateTime = " + tab.getCreateTime());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getDataLocation = " + tab.getDataLocation().toString());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getFields = " + tab.getFields());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getMetadata = " + tab.getMetadata());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getNumBuckets = " + tab.getNumBuckets());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getOutputFormatClass = " + tab.getOutputFormatClass());
                LOG.info(LOG_GY_PREFIX + " \t -\t getMetaData(QB,ReadEntity) tab.getTableSpec = " + tab.getTableSpec());
            }
            ...
            if (tab.isView()) {
                LOG.info(LOG_GY_PREFIX + " \t 4 getMetaData(QB,ReadEntity) tab.isView = " + tab.isView());
                if (qb.getParseInfo().isAnalyzeCommand()) {
                    throw new SemanticException(ErrorMsg.ANALYZE_VIEW.getMsg());
                }
                String fullViewName = tab.getDbName() + "." + tab.getTableName();
                LOG.info(LOG_GY_PREFIX + " \t 5 getMetaData(QB,ReadEntity) tab.fullViewName = " + fullViewName);
                // Prevent view cycles
                if (viewsExpanded.contains(fullViewName)) {
                    throw new SemanticException(
                            "Recursive view " + fullViewName + " detected (cycle: " + StringUtils.join(viewsExpanded,
                                                                                                       " -> ") + " -> " + fullViewName + ").");
                }
            ...
            LOG.info(LOG_GY_PREFIX + " \t 6 getMetaData(QB,ReadEntity) alias = " + alias + ", tab = "
                             + tab.getTableName());
            LOG.info(LOG_GY_PREFIX + " \t -\t ");
            LOG.info(LOG_GY_PREFIX + " \t -\t ");
            LOG.info(LOG_GY_PREFIX + " \t -\t ");
            ...
            if (qb.getParseInfo().isAnalyzeCommand()) {
                ...
                LOG.info(LOG_GY_PREFIX + " \t 7 getMetaData(QB,ReadEntity) isAnalyzeCommand ");
            ...
            if (null != parentViewInfo) {
                LOG.info(LOG_GY_PREFIX + " \t 8 getMetaData(QB,ReadEntity) accessedColumns = "
                                 + parentViewInfo.getAccessedColumns());
                LOG.info(LOG_GY_PREFIX + " \t 8 getMetaData(QB,ReadEntity) database = " + parentViewInfo.getDatabase());
                LOG.info(LOG_GY_PREFIX + " \t 8 getMetaData(QB,ReadEntity) name = " + parentViewInfo.getName());
                LOG.info(LOG_GY_PREFIX + " \t 8 getMetaData(QB,ReadEntity) path = " + parentViewInfo.getD());
                LOG.info(LOG_GY_PREFIX + " \t 8 getMetaData(QB,ReadEntity) alias = " + alias);
            }
        ...
        // 遍历子查询，并为子查询的表获得metaData
        for (String alias : qb.getSubqAliases()) {
            LOG.info(LOG_GY_PREFIX + " \t 9 getMetaData(QB,ReadEntity) subqueries = " + alias);
        ...
        LOG.info(LOG_GY_PREFIX + " \t 10 getMetaData(QB,ReadEntity). Get metadata for destination tables");
        // Go over all the destination structures and populate the related
        // metadata
        // 扫描所有的目标结构，并填充相关的metadata.
        QBParseInfo qbp = qb.getParseInfo();
        LOG.info(LOG_GY_PREFIX + " \t 11 getMetaData(QB,ReadEntity). qbp.getAlias = " + qbp.getAlias());
        LOG.info(LOG_GY_PREFIX + " \t 11-1 getMetaData(QB,ReadEntity). qbp.getClauseNames = " + qbp.getClauseNames());
        LOG.info(LOG_GY_PREFIX + " \t 11-2 getMetaData(QB,ReadEntity). qbp.getClauseNamesForDest = "
                         + qbp.getClauseNamesForDest());
        LOG.info(LOG_GY_PREFIX + " \t 11-3 getMetaData(QB,ReadEntity). qbp.getDestRollups = " + qbp.getDestRollups());
        LOG.info(LOG_GY_PREFIX + " \t 11-4 getMetaData(QB,ReadEntity). qbp.getOuterQueryLimit= "
                         + qbp.getOuterQueryLimit());

        for (String name : qbp.getClauseNamesForDest()) {
            ASTNode ast = qbp.getDestForClause(name);
            LOG.info(LOG_GY_PREFIX + " \t 12 getMetaData(QB,ReadEntity). name = " + name + ", ast-text="
                             + ast.getText());
            switch (ast.getToken().getType()) {
                case HiveParser.TOK_TAB: {

                    TableSpec ts = new TableSpec(db, conf, ast);
                    //if(null != ts.partitions){
                    //    LOG.info(LOG_GY_PREFIX + " \t 13 getMetaData(QB,ReadEntity). TOK_TAB = " + ts.partitions.toString());
                    //}
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t 13 getMetaData(QB,ReadEntity). outputFormatClass = "
                                     + outputFormatClass);
                    ...
                    boolean isTableWrittenTo = qb.getParseInfo()
                            .isInsertIntoTable(ts.tableHandle.getDbName(), ts.tableHandle.getTableName());
                    LOG.info(LOG_GY_PREFIX + " \t 13 getMetaData(QB,ReadEntity). isTableWrittenTo = "
                                     + isTableWrittenTo);
                    ...
                    assert isTableWrittenTo :
                            "Inconsistent data structure detected: we are writing to " + ts.tableHandle + " in " + name
                ...
                case HiveParser.TOK_DIR: {
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR = " + fname);
                    if ((!qb.getParseInfo().getIsSubQ()) && (((ASTNode) ast.getChild(0)).getToken().getType()
                        ...
                        if (qb.isCTAS() || qb.isMaterializedView()) {
                            ...
                            // 如果在CTAS( Create Table As Select)的查询中没有指定位置, 则使用表的位置, 或使用 db位置。
                            if (qb.getTableDesc() != null && qb.getTableDesc().getLocation() != null) {
                                ...
                                try {
                                    ...
                                    String destTableDb =
                                            qb.getTableDesc() != null ? qb.getTableDesc().getDatabaseName() : null;
                                    if (destTableDb == null) {
                                        destTableDb = names[0];
                                    }
                            ...
                            LOG.info(LOG_GY_PREFIX + " \t 14 getJMetaData(QB,ReadEntity). TOK_DIR name = " + location.getName());
                            LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR location = " + location.getParent().toString());
                            try {
                            ...
                            LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR stagingPath = "
                                             + stagingPath);
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR name = " + name);
                    LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR fname = " + fname);
                    LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR isDfsFile = " + isDfsFile);
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR numChildCount = " + numCh);
                    for (int num = 1; num < numCh; num++) {
                        ...
                        if (child != null) {
                            ...
                            LOG.info(LOG_GY_PREFIX + " \t 14 getMetaData(QB,ReadEntity). TOK_DIR child.text = "
                                             + child.toStringTree() + ", text = " + child.getText());
    ...
    private boolean isPathEncrypted(Path path) throws HiveException {
        ...
        try {
            HadoopShims.HdfsEncryptionShim hdfsEncryptionShim = SessionState.get()
                    .getHdfsEncryptionShim(path.getFileSystem(conf));
     ...
     */
    private int comparePathKeyStrength(Path p1,
                                       Path p2) throws HiveException {
    ...
    private void replaceViewReferenceWithDefinition(QB qb,
                                                    Table tab,
                                                    String tab_name,
                                                    String alias)
            throws SemanticException {
        ...
        ASTNode viewTree;
        final ASTNodeOrigin viewOrigin = new ASTNodeOrigin("VIEW",
                                                           tab.getTableName(),
        ...
        try {
            ...
            if (!unparseTranslator.isEnabled() && (viewMask.isEnabled() && analyzeRewrite == null)) {
                viewTree = rewriteASTWithMaskAndFilter(viewMask,
                                                       viewTree,
        ...
        if ((!this.skipAuthorization() && !qb.isInsideView() && HiveConf.getBoolVar(conf,
                                                                                    HiveConf.ConfVars.HIVE_AUTHORIZATION_ENABLED)) || HiveConf.getBoolVar(conf,
    ...
    private boolean isPresent(String[] list,
                              String elem) {
        for (String s : list) {
    ...
    @SuppressWarnings("nls")
    void parseJoinCondPopulateAlias(QBJoinTree joinTree,
                                    ASTNode condn,
                                    ArrayList<String> leftAliases,
                                    ArrayList<String> rightAliases,
        ...
        switch (condn.getToken().getType()) {
            ...
            case HiveParser.TOK_FUNCTION:
                ...
                for (int i = 1; i < condn.getChildCount(); i++) {
                    parseJoinCondPopulateAlias(joinTree,
                                               (ASTNode) condn.getChild(i),
                                               leftAliases,
                                               rightAliases,
                                               null,
                                               aliasToOpInfo);
            ...
            default:
                ...
                if (condn.getChildCount() == 1) {
                    ...
                    throw new SemanticException(
                            condn.toStringTree() + " encountered with " + condn.getChildCount() + " children");
                }
                break;
        }
    }
                    ...
                    if (rightCondAl1.size() != 0) {
                        ...
                        if (leftTree != null && leftTree.getNoOuterJoin()) {
                            String leftTreeRightSource = leftTree.getRightAliases() != null
                                    && leftTree.getRightAliases().length
                                    > 0 ? leftTree.getRightAliases()[0] : null;
                            ...
                            if (!leftHasRightReference && !rightHasRightReference) {
                                applyEqualityPredicateToQBJoinTree(leftTree,
                                                                   type,
                                                                   leftTreeLeftSrc,
                                                                   joinCond,
                                                                   leftCondn,
    ...
    @SuppressWarnings("rawtypes")
    private void parseJoinCondition(QBJoinTree joinTree,
                                    ASTNode joinCond,
                                    List<String> leftSrc,
                                    Map<String, Operator> aliasToOpInfo)
            throws SemanticException {
    ...
    private void parseJoinCondition(QBJoinTree joinTree,
        ...
        switch (joinCond.getToken().getType()) {
            ...
            case HiveParser.EQUAL:
                ...
                parseJoinCondPopulateAlias(joinTree, rightCondn, rightCondAl1, rightCondAl2, null, aliasToOpInfo);
                ...
                if (((leftCondAl1.size() != 0) && (leftCondAl2.size() != 0)) || ((rightCondAl1.size() != 0) && (
                        rightCondAl2.size() != 0))) {
                    joinTree.addPostJoinFilter(joinCond);
                } else {
                    applyEqualityPredicateToQBJoinTree(joinTree,
                                                       type,
                                                       leftSrc,
                                                       joinCond,
                                                       leftCondn,
            ...
            default:
                ...
                for (int ci = childrenBegin; ci < joinCond.getChildCount(); ci++) {
                    parseJoinCondPopulateAlias(joinTree,
                                               (ASTNode) joinCond.getChild(ci),
                                               leftAlias.get(ci - childrenBegin),
                                               rightAlias.get(ci - childrenBegin),
                                               null,
                                               aliasToOpInfo);
                }
    ...
    @SuppressWarnings("rawtypes")
    private void extractJoinCondsFromWhereClause(QBJoinTree joinTree,
                                                 QB qb,
                                                 String dest,
                                                 ASTNode predicate,
                                                 Map<String, Operator> aliasToOpInfo)
        ...
        switch (predicate.getType()) {
            case HiveParser.KW_AND:
                extractJoinCondsFromWhereClause(joinTree, qb, dest, (ASTNode) predicate.getChild(0), aliasToOpInfo);
                extractJoinCondsFromWhereClause(joinTree, qb, dest, (ASTNode) predicate.getChild(1), aliasToOpInfo);
                break;
            case HiveParser.EQUAL_NS:
            case HiveParser.EQUAL:
    ...
    @SuppressWarnings("nls")
    private Operator genHavingPlan(String dest,
                                   QB qb,
                                   Operator input,
                                   Map<String, Operator> aliasToOpInfo)
            throws SemanticException {
        ...
        if (!isCBOExecuted() && !qb.getParseInfo().getDestToGroupBy().isEmpty()) {
            ...
            final boolean cubeRollupGrpSetPresent = (!qb.getParseInfo().getDestRollups().isEmpty() || !qb.getParseInfo()
                    .getDestGroupingSets()
                    .isEmpty() || !qb.getParseInfo().getDestCubes().isEmpty());
            // Special handling of grouping function
            condn = rewriteGroupingFunctionAST(getGroupByForClause(qb.getParseInfo(), destClauseName),
                                               condn,
    ...
    protected static ASTNode rewriteGroupingFunctionAST(final List<ASTNode> grpByAstExprs,
                                                        ASTNode targetNode,
                        ...
                        for (int i = 0; i < grpByAstExprs.size(); i++) {
                            ...
                            if (grpByExpr.toStringTree().equals(c.toStringTree())) {
                                ...
                                if (noneSet) {
                                    ...
                                    child1 = (ASTNode) ParseDriver.adaptor.create(HiveParser.TOK_TABLE_OR_COL,
                                                                                  "TOK_TABLE_OR_COL");
    ...
    private Operator genPlanForSubQueryPredicate(QB qbSQ,
                                                 ISubQueryJoinInfo subQueryPredicate)
            throws SemanticException {
    ...
    @SuppressWarnings("nls")
    private Operator genFilterPlan(ASTNode searchCond,
                                   QB qb,
                                   Operator input,
                                   Map<String, Operator> aliasToOpInfo,
                                   boolean forHavingClause,
                                   boolean forGroupByClause)
            throws SemanticException {
        ...
        if (subQueriesInOriginalTree.size() > 0) {
            ...
            if (qb.getSubQueryPredicateDef() != null) {
                throw new SemanticException(ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(subQueriesInOriginalTree.get(
                        0), "Nested SubQuery expressions are not supported."));
            }
            ...
            if (subQueriesInOriginalTree.size() > 1) {
                ...
                throw new SemanticException(ErrorMsg.UNSUPPORTED_SUBQUERY_EXPRESSION.getMsg(subQueriesInOriginalTree.get(
                        1), "Only 1 SubQuery expression is supported."));
            }
     ...
     */
    @SuppressWarnings("nls")
    private Operator genFilterPlan(QB qb,
                                   ASTNode condn,
                                   Operator input,
                                   boolean useCaching)
            throws SemanticException {
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(new FilterDesc(filterCond, false),
     ...
     */
    private Operator genNotNullFilterForJoinSourcePlan(QB qb,
                                                       Operator input,
                                                       QBJoinTree joinTree,
        ...
        if (input instanceof FilterOperator) {
            ExprNodeDescUtils.getExprNodeColumnDesc(Arrays.asList(((FilterDesc) input.getConf()).getPredicate()),
                                                    hashes);
        }
        ...
        for (int i = 0; i < joinKeys.length; i++) {
            ...
            args.add(joinKeys[i]);
            ExprNodeDesc nextExpr = ExprNodeGenericFuncDesc.newInstance(FunctionRegistry.getFunctionInfo("isnotnull")
                                                                                .getGenericUDF(), args);
    ...
    Integer genColListRegex(String colRegex,
                            String tabAlias,
                            ASTNode sel,
                            ArrayList<ExprNodeDesc> col_list,
                            HashSet<ColumnInfo> excludeCols,
                            RowResolver input,
                            RowResolver colSrcRR,
                            Integer pos,
                            RowResolver output,
                            List<String> aliases,
        ...
        if (colSrcRR.getNamedJoinInfo() != null) {
            ...
            HashMap<String, ColumnInfo> rightMap = colSrcRR.getFieldMap(colSrcRR.getNamedJoinInfo()
                                                                                .getAliases()
                                                                                .get(1));
                    ...
                    if (ensureUniqueCols) {
                        if (!output.putWithCheck(tmp[0], tmp[1], null, oColInfo)) {
                            throw new CalciteSemanticException(
                                    "Cannot add column to RR: " + tmp[0] + "." + tmp[1] + " => " + oColInfo
        ...
        for (String alias : aliases) {
            ...
            for (Map.Entry<String, ColumnInfo> entry : fMap.entrySet()) {
                ColumnInfo colInfo = entry.getValue();
                if (colSrcRR.getNamedJoinInfo() != null && colSrcRR.getNamedJoinInfo()
                        .getNamedColumns()
                ...
                if (input != colSrcRR) {
                    ...
                    if (colInfo == null) {
                        LOG.error("Cannot find colInfo for " + tabAlias + "." + tmp[1] + ", derived from [" + colSrcRR
                                          + "], in [" + input + "]");
                ...
                if (ensureUniqueCols) {
                    if (!output.putWithCheck(tmp[0], tmp[1], null, oColInfo)) {
                        throw new CalciteSemanticException(
                                "Cannot add column to RR: " + tmp[0] + "." + tmp[1] + " => " + oColInfo
                                        + " due to duplication, see previous warnings", UnsupportedFeature.Duplicates_in_RR);
                    }
                } else {
                    output.put(tmp[0], tmp[1], oColInfo);
                }
    ...
    private TableDesc getTableDescFromSerDe(ASTNode child,
                                            String cols,
                                            String colTypes,
                                            boolean defaultCols)
            throws SemanticException {
        if (child.getType() == HiveParser.TOK_SERDENAME) {
            ...
            if (child.getChildCount() == 2) {
                ASTNode prop = (ASTNode) child.getChild(1).getChild(0);
            ...
            TableDesc tblDesc = PlanUtils.getDefaultTableDesc(Integer.toString(Utilities.ctrlaCode),
                                                              cols,
                                                              colTypes,
                                                              defaultCols);
            ...
            for (int numC = 0; numC < numChildRowFormat; numC++) {
                ...
                switch (rowChild.getToken().getType()) {
                    case HiveParser.TOK_TABLEROWFORMATFIELD:
                        ...
                        tblDesc.getProperties().setProperty(serdeConstants.SERIALIZATION_FORMAT, fieldDelim);
                    ...
                    case HiveParser.TOK_TABLEROWFORMATCOLLITEMS:
                        tblDesc.getProperties()
                                .setProperty(serdeConstants.COLLECTION_DELIM,
                                             unescapeSQLString(rowChild.getChild(0).getText()));
                        break;
                    case HiveParser.TOK_TABLEROWFORMATMAPKEYS:
                        tblDesc.getProperties()
                                .setProperty(serdeConstants.MAPKEY_DELIM,
                                             unescapeSQLString(rowChild.getChild(0).getText()));
                        break;
                    ...
                    case HiveParser.TOK_TABLEROWFORMATNULL:
                        ...
                        tblDesc.getProperties().setProperty(serdeConstants.SERIALIZATION_NULL_FORMAT, nullFormat);
                        break;
                    default:
                        assert false;
                }
    ...
    private void failIfColAliasExists(Set<String> nameSet,
    ...
    @SuppressWarnings("nls")
    private Operator genScriptPlan(ASTNode trfm,
                                   QB qb,
                                   Operator input)
            throws SemanticException {
        ...
        if (!outputColNames && !outputColSchemas) {
            ...
            if (outputColNames) {
                for (int i = 0; i < ccount; ++i) {
                    String colAlias = unescapeIdentifier(collist.getChild(i).getText()).toLowerCase();
                ...
                for (int i = 0; i < ccount; ++i) {
                    ...
                    String colAlias = unescapeIdentifier(child.getChild(0).getText()).toLowerCase();
                    failIfColAliasExists(colAliasNamesDuplicateCheck, colAlias);
                    String intName = getColumnInternalName(i);
                    ColumnInfo colInfo = new ColumnInfo(intName,
                                                        TypeInfoUtils.getTypeInfoFromTypeString(getTypeStringFromAST((ASTNode) child.getChild(1))),
                                                        null,
                                                        false);
                    colInfo.setAlias(colAlias);
                    outputCols.add(colInfo);
        ...
        if (trfm.getChild(inputSerDeNum).getChildCount() > 0) {
            inInfo = getTableDescFromSerDe((ASTNode) (((ASTNode) trfm.getChild(inputSerDeNum))).getChild(0),
                                           inpColumns.toString(),
        ...
        if (trfm.getChild(outputSerDeNum).getChildCount() > 0) {
            outInfo = getTableDescFromSerDe((ASTNode) (((ASTNode) trfm.getChild(outputSerDeNum))).getChild(0),
                                            columns.toString(),
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(new ScriptDesc(fetchFilesNotInLocalFilesystem(
                stripQuotes(trfm.getChild(execPos).getText())),
                                                                                        inInfo,
                                                                                        inRecordWriter,
                                                                                        outInfo,
                                                                                        outRecordReader,
    ...
    private Class<? extends RecordReader> getRecordReader(ASTNode node) throws SemanticException {
        ...
        try {
            return (Class<? extends RecordReader>) Class.forName(name,
                                                                 true,
                                                                 Utilities.getSessionSpecifiedClassLoader());
        } catch (ClassNotFoundException e) {
            throw new SemanticException(e);
    ...
    private Class<? extends RecordWriter> getRecordWriter(ASTNode node) throws SemanticException {
        ...
        try {
            return (Class<? extends RecordWriter>) Class.forName(name,
                                                                 true,
                                                                 Utilities.getSessionSpecifiedClassLoader());
        } catch (ClassNotFoundException e) {
            throw new SemanticException(e);
    ...
    private ObjectPair<List<ASTNode>, List<Integer>> getGroupByGroupingSetsForClause(QBParseInfo parseInfo,
                                                                                     String dest)
            throws SemanticException {
        ...
        if (root != null) {
            for (int i = 0; i < root.getChildCount(); ++i) {
                ...
                for (int j = 0; j < child.getChildCount(); ++j) {
                    ...
                    if (pos == null) {
                        throw new SemanticException(generateErrorMessage((ASTNode) child.getChild(j),
    ...
    private boolean checkForEmptyGroupingSets(List<Integer> bitmaps,
                                              int groupingIdAllSet) {
        boolean ret = true;
    ...
    public static int unsetBit(int bitmap,
                               int bitIdx) {
        return bitmap & ~(1 << bitIdx);
    }
     ...
     * a,b,c.
     */
    List<ASTNode> getGroupByForClause(QBParseInfo parseInfo,
                                      String dest) {
    ...
    static String[] getColAlias(ASTNode selExpr,
                                String defaultName,
                                RowResolver inputRR,
        ...
        if (selExpr.getChildCount() == 2 || (selExpr.getChildCount() == 3
    ...
    private Operator<?> genSelectPlan(String dest,
                                      QB qb,
                                      Operator<?> input,
                                      Operator<?> inputForSelectStar)
            throws SemanticException {
    ...
    private Operator<?> genSelectPlan(String dest,
                                      ...
                                      Operator<?> input,
                                      Operator<?> inputForSelectStar,
                                      boolean outerLV) throws SemanticException {
        ...
        for (int i = startPosn; i < exprList.getChildCount(); ++i) {
            ...
            boolean isWindowSpec =
                    child.getChildCount() == 3 && child.getChild(2).getType() == HiveParser.TOK_WINDOWSPEC;
            ...
            if (isInTransform || isUDTF) {
                ...
                String[] colRef = getColAlias(child,
            ...
            if (expr.getType() == HiveParser.TOK_ALLCOLREF) {
                ...
                selectStar = true;
            } else if (expr.getType() == HiveParser.TOK_TABLE_OR_COL && !hasAsClause && !inputRR.getIsExprResolver()
                ...
                pos = genColListRegex(unescapeIdentifier(expr.getChild(0).getText()),
                                      ...
                                      qb.getAliases(),
                                      false);
            } else if (expr.getType() == HiveParser.DOT && expr.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL
                ...
                if (!isCBOExecuted() && !qb.getParseInfo().getDestToGroupBy().isEmpty()) {
                    // If CBO did not optimize the query, we might need to replace grouping function
                    // Special handling of grouping function
                    expr = rewriteGroupingFunctionAST(getGroupByForClause(qb.getParseInfo(), dest),
                                                      expr,
                                                      !cubeRollupGrpSetPresent);
                }
                ...
                ColumnInfo colInfo = new ColumnInfo(getColumnInternalName(pos),
                                                    exp.getWritableObjectInspector(),
                                                    tabAlias,
                                                    false);
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(new SelectDesc(col_list,
                                                                                        columnNames,
                                                                                        selectStar), new RowSchema(out_rwsch.getColumnInfos()), input), out_rwsch);
     ...
     */
    public RowResolver handleInsertStatementSpec(List<ExprNodeDesc> col_list,
                                                 String dest,
        ...
        if (targetTableSchema.size() != col_list.size()) {
            ...
            throw new SemanticException(generateErrorMessage(selExprList,
                                                             "Expected " + targetTableSchema.size() + " columns for " + dest + (
                                                                     target != null ? "/" + target.getCompleteName() : (
                                                                             partition != null ? "/" + partition.getCompleteName() : "")) + "; select produces "
                                                                     + col_list.size() + " columns"));
        }
    ...
    String recommendName(ExprNodeDesc exp,
                         String colAlias) {
    ...
    public static GenericUDAFEvaluator getGenericUDAFEvaluator(String aggName,
        ...
        GenericUDAFEvaluator result = FunctionRegistry.getGenericUDAFEvaluator(aggName,
                                                                               originalParameterTypeInfos,
                                                                               isDistinct,
                                                                               isAllColumns);
        if (null == result) {
            String reason =
                    "Looking for UDAF Evaluator\"" + aggName + "\" with parameters " + originalParameterTypeInfos;
            throw new SemanticException(ErrorMsg.INVALID_FUNCTION_SIGNATURE.getMsg((ASTNode) aggTree.getChild(0),
                                                                                   reason));
        }
        return result;
    }
    ...
    public static GenericUDAFInfo getGenericUDAFInfo(GenericUDAFEvaluator evaluator,
                                                     ...
                                                     ArrayList<ExprNodeDesc> aggParameters) throws SemanticException {
    ...
    public static GenericUDAFEvaluator.Mode groupByDescModeToUDAFMode(GroupByDesc.Mode mode,
                                                                      boolean isDistinct) {
        switch (mode) {
            case COMPLETE:
     ...
     *
     * @param reduceValues value columns of the corresponding ReduceSinkOperator
     * @param internalName the internal name of this parameter
     *
     ...
     * @param genericUDAFEvaluators If not null, this function will store the mapping from Aggregation
    ...
    private Operator genGroupByPlanGroupByOperator(QBParseInfo parseInfo,
        ...
        for (Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            for (int i = 1; i < value.getChildCount(); i++) {
                ...
                if (isDistinct && lastKeyColName != null) {
                    ...
                    paraExpression =
                            Utilities.ReduceField.KEY.name() + "." + lastKeyColName + ":" + numDistinctUDFs + "."
                ...
                ExprNodeDesc reduceValue = isConstantParameterInAggregationParameters(paraExprInfo.getInternalName(),
                                                                                      reduceValues);
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(new GroupByDesc(mode,
                                                                                     outputColumnNames,
                                                                                     groupByKeys,
                                                                                     aggregations,
                                                                                     false,
                                                                                     ...
                                                                                     false,
                                                                                     -1,
                                                                                     numDistinctUDFs > 0), new RowSchema(groupByOutputRowResolver.getColumnInfos()), input),
    ...
    private void addGroupingSetKey(List<ExprNodeDesc> groupByKeys,
        ...
        String groupingSetColumnName = groupByInputRowResolver.get(null, VirtualColumn.GROUPINGID.getName())
                .getInternalName();
        ExprNodeDesc inputExpr = new ExprNodeColumnDesc(TypeInfoFactory.intTypeInfo,
                                                        groupingSetColumnName,
                                                        null,
                                                        false);
        ...
        groupByOutputRowResolver.put(null,
                                     VirtualColumn.GROUPINGID.getName(),
                                     new ColumnInfo(field, TypeInfoFactory.intTypeInfo, null, true));
        colExprMap.put(field, groupByKeys.get(groupByKeys.size() - 1));
    }
    ...
    private void processGroupingSetReduceSinkOperator(RowResolver reduceSinkInputRowResolver,
        ...
        String groupingSetColumnName = reduceSinkInputRowResolver.get(null, VirtualColumn.GROUPINGID.getName())
                .getInternalName();
        ExprNodeDesc inputExpr = new ExprNodeColumnDesc(TypeInfoFactory.intTypeInfo,
                                                        groupingSetColumnName,
                                                        null,
     ...
     * @param groupingSets                    list of grouping sets
     * @param groupingSetsPresent             whether grouping sets are present in this query
    ...
    private Operator genGroupByPlanGroupByOperator1(QBParseInfo parseInfo,
        ...
        for (Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            aggregations.add(new AggregationDesc(aggName.toLowerCase(),
                                                 udaf.genericUDAFEvaluator,
                                                 udaf.convertedParameters,
                                                 (mode != GroupByDesc.Mode.FINAL && isDistinct),
                                                 amode));
            String field = getColumnInternalName(groupByKeys.size() + aggregations.size() - 1);
            outputColumnNames.add(field);
            groupByOutputRowResolver.putExpression(value, new ColumnInfo(field, udaf.returnType, "", false));
    ...
    private Operator genGroupByPlanMapGroupByOperator(QB qb,
        ...
        for (int i = 0; i < grpByExprs.size(); ++i) {
            ...
            if ((grpByExprNode instanceof ExprNodeColumnDesc)
                    && ExprNodeDescUtils.indexOf(grpByExprNode, groupByKeys) >= 0) {
            ...
            for (ASTNode value : list) {
                ...
                for (int i = 1; i < value.getChildCount(); i++) {
                    ...
                    if (groupByOutputRowResolver.getExpression(parameter) == null) {
                        ...
                        String field = getColumnInternalName(groupByKeys.size() - 1);
                        outputColumnNames.add(field);
                        groupByOutputRowResolver.putExpression(parameter,
                                                               new ColumnInfo(field, distExprNode.getTypeInfo(), "", false));
                        colExprMap.put(field, groupByKeys.get(groupByKeys.size() - 1));
                    }
                }
            }
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(new GroupByDesc(mode,
                                                                                     ...
                                                                                     groupingSetsPresent,
                                                                                     groupingSetsPosition,
                                                                                     containsDistinctAggr), new RowSchema(groupByOutputRowResolver.getColumnInfos()), inputOperatorInfo),
                                     groupByOutputRowResolver);
        op.setColumnExprMap(colExprMap);
        return op;
    }
     ...
     *
     * @param numPartitionFields the number of fields for map-reduce partitioning. This is usually
    ...
    private ReduceSinkOperator genGroupByPlanReduceSinkOperator(QB qb,
        ...
        if (groupingSetsPresent) {
            // Process grouping set for the reduce sink operator
            processGroupingSetReduceSinkOperator(reduceSinkInputRowResolver,
                                                 reduceSinkOutputRowResolver,
                                                 reduceKeys,
                                                 outputKeyColumnNames,
                                                 colExprMap);
        ...
        List<List<Integer>> distinctColIndices = getDistinctColIndicesForReduceSink(parseInfo,
                                                                                    dest,
                                                                                    reduceKeys,
                                                                                    reduceSinkInputRowResolver,
                                                                                    reduceSinkOutputRowResolver,
        ...
        if (!mapAggrDone) {
            getReduceValuesForReduceSinkNoMapAgg(parseInfo,
                                                 dest,
                                                 reduceSinkInputRowResolver,
                                                 reduceSinkOutputRowResolver,
            ...
            for (Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
                ...
                ExprNodeColumnDesc exprDesc = new ExprNodeColumnDesc(type,
                                                                     getColumnInternalName(inputField),
                                                                     "",
                                                                     false);
        ...
        ReduceSinkOperator rsOp = (ReduceSinkOperator) putOpInsertMap(OperatorFactory.getAndMakeChild(PlanUtils.getReduceSinkDesc(
                reduceKeys,
    ...
    private void getReduceValuesForReduceSinkNoMapAgg(QBParseInfo parseInfo,
                                                      String dest,
                                                      RowResolver reduceSinkInputRowResolver,
                                                      RowResolver reduceSinkOutputRowResolver,
                                                      List<String> outputValueColumnNames,
                                                      ArrayList<ExprNodeDesc> reduceValues,
                                                      Map<String, ExprNodeDesc> colExprMap) throws SemanticException {
        HashMap<String, ASTNode> aggregationTrees = parseInfo.getAggregationExprsForClause(dest);
        ...
        for (Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            for (int i = 1; i < value.getChildCount(); i++) {
                ...
                if (reduceSinkOutputRowResolver.getExpression(parameter) == null) {
                    ...
                    String field = Utilities.ReduceField.VALUE.toString() + "." + getColumnInternalName(
                            reduceValues.size() - 1);
    ...
    @SuppressWarnings("nls")
    private ReduceSinkOperator genCommonGroupByPlanReduceSinkOperator(QB qb,
                                                                      List<String> dests,
                                                                      Operator inputOperatorInfo)
            throws SemanticException {
        ...
        ReduceSinkOperator rsOp = (ReduceSinkOperator) putOpInsertMap(OperatorFactory.getAndMakeChild(rsDesc,
                                                                                                      new RowSchema(reduceSinkOutputRowResolver.getColumnInfos()),
                                                                                                      inputOperatorInfo), reduceSinkOutputRowResolver);
        rsOp.setColumnExprMap(colExprMap);
        return rsOp;
    ...
    private void removeMappingForKeys(ASTNode predicate,
                                      Map<ASTNode, ExprNodeDesc> mapping,
    ...
    private void removeRecursively(ASTNode current,
                                   Map<ASTNode, ExprNodeDesc> mapping) {
     ...
     *
     * @param numPartitionFields the number of fields in the map-reduce partition key. This should
    ...
    private Operator genGroupByPlanReduceSinkOperator2MR(QBParseInfo parseInfo,
        ...
        if (groupingSetsPresent) {
            ...
            processGroupingSetReduceSinkOperator(reduceSinkInputRowResolver2,
        ...
        for (Map.Entry<String, ASTNode> entry : aggregationTrees.entrySet()) {
            ...
            reduceSinkOutputRowResolver2.putExpression(t,
                                                       new ColumnInfo(Utilities.ReduceField.VALUE.toString() + "." + col, typeInfo, "", false));
     ...
     * @return
     *
     * @throws SemanticException Generate a Group-By plan using 1 map-reduce job. Spray by the
     ...
     **/
    @SuppressWarnings({"nls"})
    private Operator genGroupByPlan1MR(String dest,
                                       QB qb,
                                       Operator input)
            throws SemanticException {
        ...
        int numReducers = -1;
        ObjectPair<List<ASTNode>, List<Integer>> grpByExprsGroupingSets = getGroupByGroupingSetsForClause(parseInfo,
                                                                                                          dest);
        ...
        ReduceSinkOperator reduceSinkOperatorInfo = genGroupByPlanReduceSinkOperator(qb,
                                                                                     dest,
                                                                                     input,
                                                                                     grpByExprs,
                                                                                     grpByExprs.size(),
    ...
    @SuppressWarnings({"nls"})
    private Operator genGroupByPlan1ReduceMultiGBY(List<String> dests,
                                                   QB qb,
                                                   Operator input,
                                                   Map<String, Operator> aliasToOpInfo)
            throws SemanticException {
        ...
        for (String dest : dests) {
            ...
            if (whereExpr != null) {
                ...
                ExprNodeDesc.ExprNodeDescEqualityWrapper currentWrapped = new ExprNodeDesc.ExprNodeDescEqualityWrapper(
                        current);
        ...
        if (previous != null) {
            ...
            selectInput = putOpInsertMap(OperatorFactory.getAndMakeChild(orFilterDesc,
                                                                         new RowSchema(inputRR.getColumnInfos()),
                                                                         input), inputRR);
        }
        ...
        Operator forwardOp = putOpInsertMap(OperatorFactory.getAndMakeChild(new ForwardDesc(),
                                                                            new RowSchema(reduceSinkOperatorInfoRR.getColumnInfos()),
                                                                            reduceSinkOperatorInfo), reduceSinkOperatorInfoRR);
     ...
     * @return
     *
     * @throws SemanticException Generate a Group-By plan using a 2 map-reduce jobs. Spray by the
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlan2MR(String dest,
                                       QB qb,
                                       Operator input)
            throws SemanticException {
        ...
        Operator reduceSinkOperatorInfo2 = genGroupByPlanReduceSinkOperator2MR(parseInfo,
                                                                               dest,
                                                                               groupByOperatorInfo,
                                                                               grpByExprs.size(),
                                                                               numReducers,
    ...
    private boolean optimizeMapAggrGroupBy(String dest,
                                           QB qb) throws SemanticException {
    ...
    static private boolean hasCommonElement(Set<String> set1,
                                            Set<String> set2) {
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanMapAggrNoSkew(String dest,
                                                 QB qb,
                                                 Operator inputOperatorInfo)
            throws SemanticException {
        ...
        groupOpToInputTables.put(groupByOperatorInfo,
                                 opParseCtx.get(inputOperatorInfo).getRowResolver().getTableNames());
        int numReducers = -1;
        ...
        if (!groupingSetsPresent || !groupingSetsNeedAdditionalMRJob) {
            ...
            // on the reducer.
            return genGroupByPlanGroupByOperator1(parseInfo,
                                                  dest,
                                                  reduceSinkOperatorInfo,
                                                  ...
                                                  groupingSets,
                                                  groupingSetsPresent,
                                                  groupingSetsNeedAdditionalMRJob);
        } else {
            ...
            Operator reduceSinkOperatorInfo2 = genGroupByPlanReduceSinkOperator2MR(parseInfo,
                                                                                   dest,
                                                                                   groupByOperatorInfo2,
                                                                                   grpByExprs.size() + 1,
                                                                                   numReducers,
                                                                                   groupingSetsPresent);

            // ////// Generate GroupbyOperator3
            return genGroupByPlanGroupByOperator2MR(parseInfo,
                                                    dest,
                                                    reduceSinkOperatorInfo2,
                                                    GroupByDesc.Mode.FINAL,
                                                    genericUDAFEvaluators,
     ...
     */
    @SuppressWarnings("nls")
    private Operator genGroupByPlanMapAggr2MR(String dest,
                                              QB qb,
                                              Operator inputOperatorInfo)
            throws SemanticException {
    ...
    private int getReducersBucketing(int totalFiles,
                                     int maxReducers) {
    ...
    @SuppressWarnings("nls")
    private Operator genBucketingSortingDest(String dest,
                                             Operator input,
                                             QB qb,
        ...
        if (enforceBucketing) {
            ...
            reduceSinkOperatorsAddedByEnforceBucketingSorting.add((ReduceSinkOperator) input.getParentOperators()
                    .get(0));
    ...
    private void genPartnCols(String dest,
                              Operator input,
                              QB qb,
                              TableDesc table_desc,
                              Table dest_tab,
    ...
    @SuppressWarnings("nls")
    protected Operator genFileSinkPlan(String dest,
                                       QB qb,
                                       Operator input)
            throws SemanticException {
        ...
        switch (dest_type.intValue()) {
            case QBMetaData.DEST_TABLE: {
                ...
                if ((!conf.getBoolVar(HiveConf.ConfVars.HIVE_INSERT_INTO_EXTERNAL_TABLES)) && (dest_tab.getTableType()
                ...
                if (dest_tab.isImmutable() && qb.getParseInfo()
                    ...
                    try {
                        ...
                        if (!MetaStoreUtils.isDirEmpty(fs, dest_path)) {
                            LOG.warn("Attempted write into an immutable table : " + dest_tab.getTableName() + " : "
                                             + dest_path);
                ...
                if (parts != null && parts.size() > 0) { // table is partitioned
                    ...
                    if (dpCtx == null) {
                        ...
                        dpCtx = new DynamicPartitionCtx(dest_tab,
                                                        partSpec,
                ...
                if (!isNonNativeTable) {
                    ...
                    boolean overwrite = !qb.getParseInfo()
                            .isInsertIntoTable(String.format("%s.%s", dest_tab.getDbName(), dest_tab.getTableName()));
            ...
            case QBMetaData.DEST_PARTITION: {
                ...
                if (dest_tab.isImmutable() && qb.getParseInfo()
                    ...
                    try {
                        ...
                        if (!MetaStoreUtils.isDirEmpty(fs, partPath)) {
                            LOG.warn("Attempted write into an immutable table partition : " + dest_tab.getTableName()
                                             + " : " + partPath);
                            throw new SemanticException(ErrorMsg.INSERT_INTO_IMMUTABLE_TABLE.getMsg(dest_tab.getTableName()));
                        }
                    } catch (IOException ioe) {
                ...
                dest_path = new Path(tabPath.toUri().getScheme(),
                                     tabPath.toUri().getAuthority(),
                ...
                if (!outputs.add(new WriteEntity(dest_part, determineWriteType(ltd, dest_tab.isNonNative(), dest)))) {
                    ...
                    throw new SemanticException(ErrorMsg.OUTPUT_SPECIFIED_MULTIPLE_TIMES.getMsg(
                            dest_tab.getTableName() + "@" + dest_part.getName()));
                }
                break;
            }
            ...
            case QBMetaData.DEST_DFS_FILE: {
                ...
                if (!outputs.add(new WriteEntity(dest_path, !isDfsDir, isDestTempFile))) {
                    throw new SemanticException(ErrorMsg.OUTPUT_SPECIFIED_MULTIPLE_TIMES.getMsg(dest_path.toUri()
                                                                                                        .toString()));
                }
                break;
            }
        ...
        if (updating(dest) || deleting(dest)) {
            ...
            try {
                StructObjectInspector rowObjectInspector = (StructObjectInspector) table_desc.getDeserializer(conf)
                        .getObjectInspector();
        ...
        boolean canBeMerged = (dest_tab == null || !((dest_tab.getNumBuckets() > 0) || (dest_tab.getSortCols() != null
                && dest_tab.getSortCols().size()
                > 0)));
        ...
        FileSinkDesc fileSinkDesc = new FileSinkDesc(queryTmpdir,
                                                     table_desc,
        ...
        if (isHiveServerQuery && null != table_desc && table_desc.getSerdeClassName()
                .equalsIgnoreCase(ThriftJDBCBinarySerDe.class.getName()) && HiveConf.getBoolVar(conf,
        ...
        if (ltd != null && SessionState.get() != null) {
            SessionState.get().getLineageState().mapDirToOp(ltd.getSourcePath(), output);
            ...
            SessionState.get().getLineageState().mapDirToOp(tlocation, output);
        }
        ...
        if (dest_tab != null && conf.getBoolVar(ConfVars.HIVESTATSAUTOGATHER)
                && conf.getBoolVar(ConfVars.HIVESTATSCOLAUTOGATHER) && ColumnStatsAutoGatherContext.canRunAutogatherStats(
                fso)) {
            if (dest_type.intValue() == QBMetaData.DEST_TABLE) {
                genAutoColumnStatsGatheringPipeline(qb,
                                                    table_desc,
                                                    partSpec,
                                                    input,
    ...
    private void createInsertDesc(Table table,
                                  boolean overwrite) {
        ...
        this.rootTasks.add(TaskFactory.get(new DDLWork(getInputs(), getOutputs(), preInsertTableDesc), conf));
        TaskFactory.getAndMakeChild(new DDLWork(getInputs(), getOutputs(), insertTableDesc), conf, tasks);
    }

    private void genAutoColumnStatsGatheringPipeline(QB qb,
                                                     TableDesc table_desc,
        ...
        LOG.info("Generate an operator pipeline to autogather column stats for table " + tableName + " in query "
                         + ctx.getCmd());
    ...
    // that isn't true.
    private void checkAcidConstraints(QB qb,
                                      TableDesc tableDesc,
    ...
    */
     ...
     */
    Operator genConversionSelectOperator(String dest,
                                         QB qb,
                                         Operator input,
                                         TableDesc table_desc,
                                         DynamicPartitionCtx dpCtx) throws SemanticException {
        ...
        if (!updating(dest) && !deleting(dest) && inColumnCnt != outColumnCnt) {
            String reason =
                    "Table " + dest + " has " + outColumnCnt + " columns, but query has " + inColumnCnt + " columns.";
            throw new SemanticException(ErrorMsg.TARGET_TABLE_COLUMN_MISMATCH.getMsg(qb.getParseInfo()
        ...
        if (!isMetaDataSerDe && !deleting(dest)) {
            ...
            if (updating(dest)) {
                expressions.add(new ExprNodeColumnDesc(rowFields.get(0).getType(),
                                                       rowFields.get(0).getInternalName(),
                                                       "",
                                                       true));
            }
            ...
            for (int i = 0; i < columnNumber; i++) {
                ...
                ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo,
                                                             rowFields.get(rowFieldsOffset).getInternalName(),
                                                             "",
                                                             false,
                ...
                if (!tableFieldTypeInfo.equals(rowFieldTypeInfo)) {
                    ...
                    if (column == null) {
                        String reason =
                                "Cannot convert column " + i + " from " + rowFieldTypeInfo + " to " + tableFieldTypeInfo
                                        + ".";
                        throw new SemanticException(ErrorMsg.TARGET_TABLE_COLUMN_MISMATCH.getMsg(qb.getParseInfo()
                                                                                                         .getDestForClause(dest), reason));
                    }
        ...
        if (dynPart && dpCtx != null && dpCtx.getNumDPCols() > 0) {
            ...
            for (int i = tableFields.size() + (updating(dest) ? 1 : 0); i < rowFields.size(); ++i) {
                ...
                ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo,
                                                             rowFields.get(i).getInternalName(),
                                                             "",
                                                             true);
                expressions.add(column);
            }
        ...
        if (converted) {
            ...
            input = putOpInsertMap(OperatorFactory.getAndMakeChild(new SelectDesc(expressions, colNames),
                                                                   new RowSchema(rowResolver.getColumnInfos()),
                                                                   input), rowResolver);
    ...
    @SuppressWarnings("nls")
    private Operator genLimitPlan(String dest,
                                  QB qb,
                                  Operator input,
                                  int offset,
                                  int limit) {
        ...
        Operator limitMap = putOpInsertMap(OperatorFactory.getAndMakeChild(limitDesc,
                                                                           new RowSchema(inputRR.getColumnInfos()),
                                                                           input), inputRR);
    ...
    private Operator genUDTFPlan(GenericUDTF genericUDTF,
        ...
        StandardStructObjectInspector rowOI = ObjectInspectorFactory.getStandardStructObjectInspector(colNames,
                                                                                                      Arrays.asList(colOIs));
        ...
        if (numUdtfCols != numSuppliedAliases) {
            throw new SemanticException(ErrorMsg.UDTF_ALIAS_MISMATCH.getMsg(
                    "expected " + numUdtfCols + " aliases " + "but got " + numSuppliedAliases));
        }
        ...
        Operator<?> udtf = putOpInsertMap(OperatorFactory.getAndMakeChild(new UDTFDesc(genericUDTF, outerLV),
    ...
    @SuppressWarnings("nls")
    private Operator genLimitMapRedPlan(String dest,
                                        QB qb,
                                        Operator input,
                                        int offset,
                                        int limit,
    ...
    private ArrayList<ExprNodeDesc> getPartitionColsFromBucketCols(String dest,
                                                                   QB qb,
                                                                   Table tab,
                                                                   TableDesc table_desc,
                                                                   Operator input,
    ...
    private ArrayList<ExprNodeDesc> getPartitionColsFromBucketColsForUpdateDelete(Operator input,
                                                                                  boolean convert)
            throws SemanticException {
        ...
        ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo,
                                                     rowField.getInternalName(),
                                                     rowField.getTabAlias(),
                                                     true);
        if (convert) {
    ...
    private ArrayList<ExprNodeDesc> genConvertCol(String dest,
                                                  ...
                                                  Operator input,
                                                  List<Integer> posns,
                                                  boolean convert) throws SemanticException {
        StructObjectInspector oi = null;
        try {
            Deserializer deserializer = table_desc.getDeserializerClass().newInstance();
            SerDeUtils.initializeSerDe(deserializer, conf, table_desc.getProperties(), null);
        ...
        for (Integer posn : posns) {
            ...
            ExprNodeDesc column = new ExprNodeColumnDesc(rowFieldTypeInfo,
                                                         rowFields.get(posn).getInternalName(),
                                                         rowFields.get(posn).getTabAlias(),
            ...
            if (convert && !tableFieldTypeInfo.equals(rowFieldTypeInfo)) {
                ...
                if (column == null) {
                    String reason =
                            "Cannot convert column " + posn + " from " + rowFieldTypeInfo + " to " + tableFieldTypeInfo
                                    + ".";
                    throw new SemanticException(ErrorMsg.TARGET_TABLE_COLUMN_MISMATCH.getMsg(qb.getParseInfo()
                                                                                                     .getDestForClause(dest), reason));
                }
    ...
    private Operator genReduceSinkPlan(String dest,
                                       QB qb,
                                       Operator<?> input,
                                       int numReducers,
                                       boolean hasOrderBy)
        ...
        Operator result = genReduceSinkPlan(input,
                                            partCols,
                                            sortCols,
                                            order.toString(),
                                            nullOrder.toString(),
                                            ...
                                            Operation.NOT_ACID,
                                            true);
        if (result.getParentOperators().size() == 1 && result.getParentOperators()
                .get(0) instanceof ReduceSinkOperator) {
    ...
    private Operator genReduceSinkPlan(Operator<?> input,
                                       ...
                                       String nullOrder,
                                       int numReducers,
                                       AcidUtils.Operation acidOp) throws SemanticException {
        return genReduceSinkPlan(input, partitionCols, sortCols, sortOrder, nullOrder, numReducers, acidOp, false);
    }
    ...
    private Operator genReduceSinkPlan(Operator<?> input,
                                       ...
                                       String nullOrder,
                                       int numReducers,
                                       AcidUtils.Operation acidOp,
                                       boolean pullConstants)
            throws SemanticException {
        ...
        ReduceSinkDesc rsdesc = PlanUtils.getReduceSinkDesc(newSortCols,
                                                            valueCols,
                                                            outputColumns,
                                                            false,
                                                            -1,
                                                            partitionCols,
                                                            newSortOrder.toString(),
                                                            newNullOrder.toString(),
                                                            numReducers,
                                                            acidOp);
        Operator interim = putOpInsertMap(OperatorFactory.getAndMakeChild(rsdesc,
                                                                          new RowSchema(rsRR.getColumnInfos()),
                                                                          input), rsRR);
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(select,
                                                                         new RowSchema(selectRR.getColumnInfos()),
                                                                         interim), selectRR);
        output.setColumnExprMap(selColExprMap);
        return output;
    }

    private Operator genJoinOperatorChildren(QBJoinTree join,
                                             Operator left,
                                             Operator[] right,
        ...
        JoinDesc desc = new JoinDesc(exprMap,
                                     outputColumnNames,
                                     join.getNoOuterJoin(),
                                     joinCondns,
                                     filterMap,
        ...
        JoinOperator joinOp = (JoinOperator) OperatorFactory.getAndMakeChild(getOpContext(),
                                                                             desc,
                                                                             new RowSchema(outputRR.getColumnInfos()),
                                                                             rightOps);
    ...
    private ExprNodeDesc[][] genJoinKeys(QBJoinTree joinTree,
                                         Operator[] inputs) throws SemanticException {
    ...
    @SuppressWarnings("nls")
    private Operator genJoinReduceSinkChild(QB qb,
                                            ExprNodeDesc[] joinKeys,
                                            Operator<?> child,
                                            String[] srcs,
    ...
    private Operator genJoinOperator(QB qb,
                                     QBJoinTree joinTree,
    ...
    private Operator insertSelectForSemijoin(ArrayList<ASTNode> fields,
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(new SelectDesc(colList,
                                                                                        outputColumnNames,
                                                                                        false), new RowSchema(outputRR.getColumnInfos()), input), outputRR);
    ...
    private Operator genMapGroupByForSemijoin(QB qb,
        ...
        Operator op = putOpInsertMap(OperatorFactory.getAndMakeChild(new GroupByDesc(mode,
                                                                                     ...
                                                                                     false,
                                                                                     -1,
                                                                                     false), new RowSchema(groupByOutputRowResolver.getColumnInfos()), inputOperatorInfo),
                                     groupByOutputRowResolver);
    ...
    private ExprNodeDesc[][] genJoinOperatorTypeCheck(ExprNodeDesc[][] keys) throws SemanticException {
        ...
        for (int k = 0; k < keyLength; k++) {
            ...
            for (int i = 1; i < keys.length; i++) {
                ...
                if (commonType == null) {
                    throw new SemanticException(
                            "Cannot do equality join on different types: " + a.getTypeName() + " and "
                                    + b.getTypeName());
                }
            }
    ...
    private Operator genJoinPlan(QB qb,
     ...
     */
    private void pushJoinFilters(QB qb,
                                 QBJoinTree joinTree,
                                 Map<String, Operator> map) throws SemanticException {
            ...
            if (((ASTNode) hint.getChild(0)).getToken().getType() == HintParser.TOK_MAPJOIN) {
                ...
                if (!conf.getBoolVar(HiveConf.ConfVars.HIVEIGNOREMAPJOINHINT)
                    ...
                    for (int tblPos = 0; tblPos < numCh; tblPos++) {
                        String tblName = hintTblNames.getChild(tblPos).getText().toLowerCase();
    ...
    // the sub-query alias.
    private String getModifiedAlias(QB qb,
                                    String alias) {
        return QB.getAppendedAliasFromId(qb.getId(), alias);
    }

    private QBJoinTree genUniqueJoinTree(QB qb,
                                         ASTNode joinParseTree,
                                         Map<String, Operator> aliasToOpInfo)
            throws SemanticException {
        ...
        for (int i = 0; i < joinParseTree.getChildCount(); i++) {
            ...
            switch (child.getToken().getType()) {
                case HiveParser.TOK_TABREF:
                    ...
                    String alias = child.getChildCount() == 1 ? tableName : unescapeIdentifier(child.getChild(
                            child.getChildCount() - 1).getText().toLowerCase());
                ...
                case HiveParser.TOK_EXPLIST:
                    if (cols == -1 && child.getChildCount() != 0) {
                        ...
                        throw new SemanticException(
                                "Tables with different or invalid " + "number of keys in UNIQUEJOIN");
                    }
        ...
        if ((qb.getParseInfo().getHints() != null) && !(conf.getVar(HiveConf.ConfVars.HIVE_EXECUTION_ENGINE)
                .equals("tez"))) {
     ...
     */
    private QBJoinTree genSQJoinTree(QB qb,
                                     ISubQueryJoinInfo subQuery,
                                     Operator joiningOp,
    ...
    private QBJoinTree genJoinTree(QB qb,
                                   ASTNode joinParseTree,
                                   Map<String, Operator> aliasToOpInfo)
            throws SemanticException {

        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " genJoinTree(QB,ASTNode,Map<String, Operator>) ");
        QBJoinTree joinTree = new QBJoinTree();
        JoinCond[] condn = new JoinCond[1];
        ...
        if (!isValidLeftToken && !isJoinLeftToken) {
            throw new SemanticException("Invalid token on the left side of the join: " + left.getToken().getText()
            ...
            throw new SemanticException("Invalid token on the right side of the join: " + right.getToken().getText()
                                                + "; please rewrite your query" + advice);
        }

        LOG.info(LOG_GY_PREFIX + " \t 1 genJoinTree(QB,ASTNode,Map<String, Operator>) isValidLeftToken = "
                         + isValidLeftToken);
        LOG.info(LOG_GY_PREFIX + " \t 2 genJoinTree(QB,ASTNode,Map<String, Operator>) isJoinLeftToken = "
                         + isJoinLeftToken);
        LOG.info(LOG_GY_PREFIX + " \t 3 genJoinTree(QB,ASTNode,Map<String, Operator>) isValidRightToken = "
                         + isValidRightToken);

        if (isValidLeftToken) {
            LOG.info(LOG_GY_PREFIX + " \t 4 genJoinTree(QB,ASTNode,Map<String, Operator>) isValidLeftToken = "
                             + isValidLeftToken);
            ...
            LOG.info(LOG_GY_PREFIX + " \t 5 genJoinTree(QB,ASTNode,Map<String, Operator>) isJoinLeftToken = "
                             + isJoinLeftToken);
        ...
        if (isValidRightToken) {
            LOG.info(LOG_GY_PREFIX + " \t 6 genJoinTree(QB,ASTNode,Map<String, Operator>) isValidRightToken = "
                             + isValidRightToken);
        ...
        LOG.info(LOG_GY_PREFIX + " \t 7 genJoinTree(QB,ASTNode,Map<String, Operator>) getLeftAliases");
        String[] leftAliases = joinTree.getLeftAliases();
        if (null != leftAliases) {
            for (String s : leftAliases) {
                LOG.info(LOG_GY_PREFIX + " \t genJoinTree(QB,ASTNode,Map<String, Operator>) left-alias = " + s);
            }
        }

        LOG.info(LOG_GY_PREFIX + " \t 8 genJoinTree(QB,ASTNode,Map<String, Operator>) getRightAliases = ");
        String[] rightAliases = joinTree.getLeftAliases();
        if (null != rightAliases) {
            for (String s : rightAliases) {
                LOG.info(LOG_GY_PREFIX + " \t genJoinTree(QB,ASTNode,Map<String, Operator>) right-alias = " + s);
            }
        }

        LOG.info(LOG_GY_PREFIX + " \t 9 genJoinTree(QB,ASTNode,Map<String, Operator>) getMapAliases = "
                         + joinTree.getMapAliases());
        LOG.info(LOG_GY_PREFIX + " \t 10 genJoinTree(QB,ASTNode,Map<String, Operator>) getJoinSrc = "
                         + joinTree.getJoinSrc());

        LOG.info(LOG_GY_PREFIX + LOG_GY_END + "genJoinTree(QB,ASTNode,Map<String, Operator>) ");
        return joinTree;
    }

    private static boolean isValidJoinSide(ASTNode right) {
        return (right.getToken().getType() == HiveParser.TOK_TABREF) || (right.getToken().getType()
                == HiveParser.TOK_SUBQUERY) || (
                right.getToken().getType() == HiveParser.TOK_PTBLFUNCTION);
    }

    private String extractJoinAlias(ASTNode node,
                                    String tableName) {
        // ptf node form is:
    ...
    private void parseStreamTables(QBJoinTree joinTree,
                                   QB qb) {
     ...
     */
    private void mergeJoins(QB qb,
                            QBJoinTree node,
                            QBJoinTree target,
                            int pos,
                            int[] tgtToNodeExprMap) {
    ...
    private ObjectPair<Integer, int[]> findMergePos(QBJoinTree node,
                                                    QBJoinTree target) {
        int res = -1;
    ...
    private void mergeJoinTree(QB qb) {
        ...
        for (int i = trees.size() - 1; i >= 0; i--) {
            ...
            for (int j = i - 1; j >= 0 && continueScanning; j--) {
                ...
                if ((!node.getNoOuterJoin() && node.getPostJoinFilters().size() != 0) || (!target.getNoOuterJoin() &&
                        target.getPostJoinFilters()
                                .size() != 0)) {
    ...
    private Operator genSelectAllDesc(Operator input) {
        ...
        Operator output = putOpInsertMap(OperatorFactory.getAndMakeChild(new SelectDesc(colList, columnNames, true),
                                                                         outputRR.getRowSchema(),
                                                                         input), outputRR);
    ...
    private void combineExprNodeLists(List<ExprNodeDesc> list,
                                      List<ExprNodeDesc> list2,
    ...
    private boolean matchExprLists(List<ExprNodeDesc> list1,
                                   List<ExprNodeDesc> list2) {
    ...
    private List<ExprNodeDesc> getDistinctExprs(QBParseInfo qbp,
                                                String dest,
                                                RowResolver inputRR)
            throws SemanticException {
    ...
    @SuppressWarnings("nls")
    private Operator genBodyPlan(QB qb,
                                 Operator input,
                                 Map<String, Operator> aliasToOpInfo)
            throws SemanticException {
        QBParseInfo qbp = qb.getParseInfo();
        ...
        if (!commonGroupByDestGroups.isEmpty()) {
            ...
            for (List<String> commonGroupByDestGroup : commonGroupByDestGroups) {
                ...
                if (commonGroupByDestGroup.size() == 1 || (qbp.getAggregationExprsForClause(firstDest).size() == 0
                    ...
                    for (String dest : commonGroupByDestGroup) {
                        ...
                        if (qbp.getWhrForClause(dest) != null) {
                            ASTNode whereExpr = qb.getParseInfo().getWhrForClause(dest);
                            curr = genFilterPlan((ASTNode) whereExpr.getChild(0),
                                                 qb,
                                                 curr,
                                                 aliasToOpInfo,
                                                 false,
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("RR before GB " + opParseCtx.get(gbySource).getRowResolver() + " after GB "
                                              + opParseCtx.get(curr).getRowResolver());
                        }
    ...
    private Operator genPostGroupByBodyPlan(Operator curr,
                                            String dest,
                                            QB qb,
                                            Map<String, Operator> aliasToOpInfo,
                                            Operator gbySource) throws SemanticException {
        ...
        if (qbp.getIsSubQ()) {
            ...
            if (limit != null) {
                ...
                if (hasOrderBy || qb.getIsQuery() && qbp.getClusterByForClause(dest) == null
    ...
    @SuppressWarnings("nls")
    private Operator genUnionPlan(String unionalias,
                                  String leftalias,
                                  Operator leftOp,
                                  String rightalias,
                                  Operator rightOp)
        ...
        while (lIter.hasNext()) {
            ...
            if (commonTypeInfo == null) {
                throw new SemanticException(generateErrorMessage(tabref,
                                                                 "Schema of both sides of union should match: Column " + field + " is of type " + lInfo.getType()
                                                                         .getTypeName() + " on first table and type " + rInfo.getType().getTypeName()
                                                                         + " on second table"));
            }
        ...
        if (leftOp instanceof UnionOperator || (leftOp instanceof SelectOperator && leftOp.getParentOperators() != null
                && !leftOp.getParentOperators().isEmpty() && leftOp.getParentOperators()
                .get(0) instanceof UnionOperator
     ...
     *
     * @param origInputOp       The original input operator to the union.
     * @param origInputFieldMap A map from field name to ColumnInfo for the original input operator.
     * @param origInputAlias    The alias associated with the original input operator.
     * @param unionoutRR        The union's output row resolver.
     * @param unionalias        The alias of the union.
     *
     * @return
     *
     * @throws SemanticException
    ...
    private Operator<? extends OperatorDesc> genInputSelectForUnion(Operator<? extends OperatorDesc> origInputOp,
                                                                    Map<String, ColumnInfo> origInputFieldMap,
                                                                    String origInputAlias,
                                                                    RowResolver unionoutRR,
        ...
        while (oIter.hasNext()) {
            ...
            ExprNodeDesc column = new ExprNodeColumnDesc(oInfo.getType(),
                                                         oInfo.getInternalName(),
                                                         oInfo.getTabAlias(),
     ...
     * @param useBucketCols Flag to indicate whether the bucketCols should be used as input to
     *                      the hash function
     ...
     * @param qbm           The metadata information for the query block which is used to
     *                      resolve unaliased columns
     * @param planExpr      The plan tree for the expression. If the user specified this, the
    ...
    private ExprNodeDesc genSamplePredicate(TableSample ts,
                                            ...
                                            boolean useBucketCols,
                                            String alias,
                                            RowResolver rwsch,
                                            QBMetaData qbm,
                                            ExprNodeDesc planExpr)
            throws SemanticException {

        ExprNodeDesc numeratorExpr = new ExprNodeConstantDesc(TypeInfoFactory.intTypeInfo,
                                                              Integer.valueOf(ts.getNumerator() - 1));

        ExprNodeDesc denominatorExpr = new ExprNodeConstantDesc(TypeInfoFactory.intTypeInfo,
                                                                Integer.valueOf(ts.getDenominator()));

        ExprNodeDesc intMaxExpr = new ExprNodeConstantDesc(TypeInfoFactory.intTypeInfo,
        ...
        ExprNodeDesc equalsExpr = null;
        {
            ...
            ExprNodeDesc andExpr = TypeCheckProcFactory.DefaultExprProcessor.getFuncExprNodeDesc("&",
                                                                                                 hashfnExpr,
                                                                                                 intMaxExpr);
            assert (andExpr != null);
            LOG.info("andExpr = " + andExpr);
            ExprNodeDesc modExpr = TypeCheckProcFactory.DefaultExprProcessor.getFuncExprNodeDesc("%",
                                                                                                 andExpr,
                                                                                                 denominatorExpr);
            assert (modExpr != null);
    ...
    protected String getAliasId(String alias,
                                QB qb) {
        return (qb.getId() == null ? alias : qb.getId() + ":" + alias).toLowerCase();
    }

    @SuppressWarnings("nls")
    private Operator genTablePlan(String alias,
                                  QB qb) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " SemanticAnalyzer.genPlan(QB,boolean) ");

        String alias_id = getAliasId(alias, qb);
        Table tab = qb.getMetaData().getSrcForAlias(alias);

        LOG.info(LOG_GY_PREFIX + " \t 1 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias + ", alias_id="
                         + alias_id);
        LOG.info(LOG_GY_PREFIX + " \t 2 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                         + ", tab.getCompleteName=" + tab.getCompleteName());
        LOG.info(LOG_GY_PREFIX + " \t 2-1 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                         + ", tab.getPartitionKeys=" + tab.getPartitionKeys());
        LOG.info(
                LOG_GY_PREFIX + " \t 2-2 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias + ", tab.getDbName="
                        + tab.getDbName());
        LOG.info(LOG_GY_PREFIX + " \t 2-3 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias + ", tab.getOwner="
                         + tab.getOwner());
        LOG.info(LOG_GY_PREFIX + " \t 2-4 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias + ", tab.getCols="
                         + tab.getCols());
        LOG.info(LOG_GY_PREFIX + " \t 2-5 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                         + ", tab.getAllCols=" + tab.getAllCols());
        LOG.info(LOG_GY_PREFIX + " \t 2-6 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                         + ", tab.getPartColNames=" + tab.getPartColNames());
        LOG.info(LOG_GY_PREFIX + " \t 2-7 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                         + ", tab.getPartCols=" + tab.getPartCols());
        LOG.info(LOG_GY_PREFIX + " \t 2-8 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                         + ", tab.getPartColNames=" + tab.getPartColNames());
        RowResolver rwsch;
        ...
        LOG.info(LOG_GY_PREFIX + " \t 3 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias + ", properties="
                         + properties);

        if (top == null) {
            // Determine row schema for TSOP<TableScanOperator>.
            ...
            try {
                ...
                List<? extends StructField> fields = rowObjectInspector.getAllStructFieldRefs();
                LOG.info(LOG_GY_PREFIX + " \t SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                                 + ", all-fields=" + fields);
                for (int i = 0; i < fields.size(); i++) {
                    /**
                    ...
                    LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                                     + ", fieldName = " + fields.get(i).getFieldName() + ", colInfo=" + colInfo);
            ...
            while (vcs.hasNext()) {
                ...
                LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genTablePlan(String,QB) VirtualColumn.getName  = "
                                 + vc.getName() + ", getTypeName = " + vc.getTypeInfo().getTypeName());
            }
            ...
            LOG.info(LOG_GY_PREFIX + " \t 4 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias + ", RowSchema ="
                             + top.getSchema());
            ...
            LOG.info(LOG_GY_PREFIX + " \t 5-0 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                             + ", top.getNeededColumns=" + top.getNeededColumns());
            LOG.info(LOG_GY_PREFIX + " \t 5-1 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                             + ", top.getNeededColumnIDs=" + top.getNeededColumnIDs());
            LOG.info(LOG_GY_PREFIX + " \t 5-2 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                             + ", top.getName=" + top.getName());
            LOG.info(LOG_GY_PREFIX + " \t 5-3 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                             + ", top.getSchemaEvolutionColumns=" + top.getSchemaEvolutionColumns());
            LOG.info(LOG_GY_PREFIX + " \t 5-4 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                             + ", top.getSchemaEvolutionColumnsTypes=" + top.getSchemaEvolutionColumnsTypes());
            LOG.info(LOG_GY_PREFIX + " \t 5-5 SemanticAnalyzer.genTablePlan(String,QB) alias = " + alias
                             + ", top.getReferencedColumns=" + top.getReferencedColumns());
        } else {
        ...
        if (ts != null) {
            ...
            LOG.info(LOG_GY_PREFIX + " \t 6 SemanticAnalyzer.genTablePlan(String,QB) ");
            ...
            if (num > den) {
                throw new SemanticException(
                        ErrorMsg.BUCKETED_NUMERATOR_BIGGER_DENOMINATOR.getMsg() + " " + tab.getTableName());
            }
            ...
            for (int i = 0; i < sampleExprs.size() && colsEqual; i++) {
                LOG.info(LOG_GY_PREFIX + " \t 7 SemanticAnalyzer.genTablePlan(String,QB) ");
            ...
            if ((sampleExprs == null || sampleExprs.size() == 0 || colsEqual) && (num == den || (den % numBuckets == 0
                    || numBuckets % den
                    == 0))) {
                ...
                LOG.info(LOG_GY_PREFIX + " \t 8 SemanticAnalyzer.genTablePlan(String,QB) ");
                ExprNodeDesc samplePredicate = genSamplePredicate(ts,
                                                                  ...
                                                                  qb.getMetaData(),
                                                                  null);
                FilterDesc filterDesc = new FilterDesc(samplePredicate,
                                                       true,
                                                       new SampleDesc(ts.getNumerator(), ts.getDenominator(), tabBucketCols, true));
                filterDesc.setGenerated(true);
                ...
                LOG.info(LOG_GY_PREFIX + " \t 9 SemanticAnalyzer.genTablePlan(String,QB) ");
                ExprNodeDesc samplePredicate = genSamplePredicate(ts,
                                                                  tabBucketCols,
                                                                  colsEqual,
                                                                  alias,
            ...
            if (testMode) {
                ...
                LOG.info(LOG_GY_PREFIX + " \t 10 SemanticAnalyzer.genTablePlan(String,QB) ");
                ...
                if (!unsample) {
                    ...
                    if (numBuckets > 0) {
                        LOG.info(LOG_GY_PREFIX + " \t 11 SemanticAnalyzer.genTablePlan(String,QB) ");
                        TableSample tsSample = new TableSample(1, numBuckets);
                        tsSample.setInputPruning(true);
                        qb.getParseInfo().setTabSample(alias, tsSample);
                        ExprNodeDesc samplePred = genSamplePredicate(tsSample,
                                                                     tab.getBucketCols(),
                                                                     true,
                                                                     alias,
                                                                     rwsch,
                                                                     qb.getMetaData(),
                                                                     null);
                        FilterDesc filterDesc = new FilterDesc(samplePred,
                                                               true,
                                                               new SampleDesc(tsSample.getNumerator(),
                                                                              tsSample.getDenominator(),
                                                                              tab.getBucketCols(),
                                                                              true));
                        filterDesc.setGenerated(true);
                        op = OperatorFactory.getAndMakeChild(filterDesc, new RowSchema(rwsch.getColumnInfos()), top);
                        ...
                        LOG.info(LOG_GY_PREFIX + " \t 12 SemanticAnalyzer.genTablePlan(String,QB) ");
                        ...
                        qb.getParseInfo().setTabSample(alias, tsSample);
                        LOG.info("Need sample filter");
                        ExprNodeDesc randFunc = TypeCheckProcFactory.DefaultExprProcessor.getFuncExprNodeDesc("rand",
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " SemanticAnalyzer.genPlan(QB,boolean) ");
        return output;
    }

    static boolean isSkewedCol(String alias,
                               QB qb,
                               String colName) {
    ...
    private void setupStats(TableScanDesc tsDesc,
                            QBParseInfo qbp,
                            Table tab,
                            String alias,
                            RowResolver rwsch)
        ...
        if ((!qbp.isAnalyzeCommand() && qbp.getAnalyzeRewrite() == null) || (qbp.getAnalyzeRewrite() != null
                && !HiveConf.getVar(conf,
                                    HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("tez"))) {

            LOG.info(LOG_GY_PREFIX
                             + " \t 1 SemanticAnalyzer.setupStats(TableScanDesc,QBParseInfo,Table,String,RowResolver) ");
            tsDesc.setGatherStats(false);
        } else {

            LOG.info(LOG_GY_PREFIX
                             + " \t 2 SemanticAnalyzer.setupStats(TableScanDesc,QBParseInfo,Table,String,RowResolver) ");
            ...
            LOG.info(LOG_GY_PREFIX
                             + " \t 2 SemanticAnalyzer.setupStats(TableScanDesc,QBParseInfo,Table,String,RowResolver) tblName = "
                             + tblName);
    ...
    private Operator genPlan(QB parent,
        ...
        if (qbexpr.getOpcode() == QBExpr.Opcode.UNION) {
            ...
            return genUnionPlan(qbexpr.getAlias(),
    ...
    public Operator genPlan(QB qb) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " SemanticAnalyzer.genPlan(QB) ");
        Operator op = genPlan(qb, false);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " SemanticAnalyzer.genPlan(QB) ");
        return op;
    }

    @SuppressWarnings("nls")
    public Operator genPlan(QB qb,
                            boolean skipAmbiguityCheck) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " SemanticAnalyzer.genPlan(QB,boolean) ");
        LOG.info(LOG_GY_PREFIX + " \t 1 SemanticAnalyzer.genPlan(QB,boolean) skipAmbiguityCheck = "
                         + skipAmbiguityCheck);

        // First generate all the opInfos for the elements in the from clause
        // Must be deterministic order map - see HIVE-8707
        // 首先为 from 子句中的元素成生操作信息.
        // 必须 先确定顺序图.
        Map<String, Operator> aliasToOpInfo = new LinkedHashMap<String, Operator>();

        // Recurse over the subqueries to fill the subquery part of the plan
        // 递归子查询来填充 查询计划的 子查询部分.
        for (String alias : qb.getSubqAliases()) {
            LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genPlan(QB,boolean) alias = " + alias);
            QBExpr qbexpr = qb.getSubqForAlias(alias);
            LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genPlan(QB,boolean) alias = " + alias + ", qbexpr = "
                             + qbexpr.toString());

            Operator operator = genPlan(qb, qbexpr);

            LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genPlan(QB,boolean) alias = " + alias + ", operator = "
                             + operator.getType());
            LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genPlan(QB,boolean) alias = " + alias
                             + ", reduce-output-name = " + operator.getReduceOutputName());
            LOG.info(LOG_GY_PREFIX + " \t \t -SemanticAnalyzer.genPlan(QB,boolean) alias = " + alias + ", schema = "
                             + operator.getSchema());
            ...
            if (qb.getViewToTabSchema().containsKey(alias)) {
                ...
                // 我们设置 viewProjectToTableSchema 以便我们可以利用 ColumnPruner(列修剪工具)
                if (operator instanceof SelectOperator) {
                    ...
                    throw new SemanticException("View " + alias + " is corresponding to " + operator.getType().name()
                                                        + ", rather than a SelectOperator.");
                }
            }
        }
        ...
        // 递归所有的源表.
        for (String alias : qb.getTabAliases()) {
            LOG.info(LOG_GY_PREFIX + " \t \t >>>>>>>>>>>>>>>>>>>>>>>>>>>> SemanticAnalyzer.genPlan(QB,boolean) alias = "
                             + alias);
        ...
        if (queryProperties.hasPTF()) {
            ...
            // 处理完 子查询(subquery) 和 源表(source table)之后，处理分区表函数.
        ...
        if (qb.getParseInfo().getJoinExpr() != null) {
            LOG.info(LOG_GY_PREFIX + " \t SemanticAnalyzer.genPlan(QB,boolean) join-expr = " + qb.getParseInfo()
                    .getJoinExpr()
                    .toStringTree());
            ASTNode joinExpr = qb.getParseInfo().getJoinExpr();

            if (joinExpr.getToken().getType() == HiveParser.TOK_UNIQUEJOIN) {
                LOG.info(LOG_GY_PREFIX + " \t SemanticAnalyzer.genPlan(QB,boolean) UniqueJoinTree ");
                QBJoinTree joinTree = genUniqueJoinTree(qb, joinExpr, aliasToOpInfo);
                qb.setQbJoinTree(joinTree);
            } else {
                LOG.info(LOG_GY_PREFIX + " \t SemanticAnalyzer.genPlan(QB,boolean) JoinTree");
                 ...
                 * as Join conditions
                 * 如果查询中只有一个目的，尝试将where谓词做为Join条件.
                 */
            ...
            // 现在如果有多于1个源表，而后面有一个join示例，那么我们可以像union一样扩大查询
    ...
    private void rewriteRRForSubQ(String alias,
                                  Operator operator,
     ...
     *
     * @param aliasToOpInfo A mapping from a table alias to the TS operator. This function
    ...
    void genLateralViewPlans(Map<String, Operator> aliasToOpInfo,
                             QB qb) throws SemanticException {
    ...
    private Operator genLateralViewPlanForDest(String dest,
                                               QB qb,
    ...
    private Operator genLateralViewPlan(QB qb,
                                        Operator op,
                                        ASTNode lateralViewTree) throws SemanticException {
        ...
        Operator lvForward = putOpInsertMap(OperatorFactory.getAndMakeChild(new LateralViewForwardDesc(),
                                                                            new RowSchema(lvForwardRR.getColumnInfos()),
                                                                            op), lvForwardRR);
        ...
        Operator allPath = putOpInsertMap(OperatorFactory.getAndMakeChild(sDesc,
        ...
        Operator udtfPath = genSelectPlan(null,
                                          (ASTNode) lateralViewTree.getChild(0),
                                          blankQb,
                                          lvForward,
                                          null,
        ...
        Operator lateralViewJoin = putOpInsertMap(OperatorFactory.getAndMakeChild(new LateralViewJoinDesc(allColumns,
                                                                                                          outputInternalColNames), new RowSchema(lateralViewRR.getColumnInfos()), allPath, udtfPath),
                                                  lateralViewRR);
     ...
     * @param dest
     * @param colExprMap
     * @param outputInternalColNames - a list to which the new internal column names will be added, in
     *                               the same order as in the dest row resolver
     */
    private void LVmergeRowResolvers(RowResolver source,
                                     RowResolver dest,
                                     Map<String, ExprNodeDesc> colExprMap,
        ...
        for (ColumnInfo c : source.getColumnInfos()) {
            ...
            ColumnInfo newCol = new ColumnInfo(internalName,
                                               c.getType(),
                                               c.getTabAlias(),
                                               c.getIsVirtualCol(),
                                               c.isHiddenVirtualCol());
    ...
    public Phase1Ctx initPhase1Ctx() {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " SemanticAnalyzer.initPhase1Ctx ");
        Phase1Ctx ctx_1 = new Phase1Ctx();
        ctx_1.nextNum = 0;
        ctx_1.dest = "reduce";
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " SemanticAnalyzer.initPhase1Ctx ");
        return ctx_1;
    }
    ...
    public void init(boolean clearPartsCache) {
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 SemanticAnalyzer.init(boolean) clearPartsCache = " + clearPartsCache);
        // clear most members
        ...
        this.qb = qb;
        LOG.info(LOG_GY_PREFIX + " \t 2 SemanticAnalyzer.init(boolean) Create QB Object.");
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getAlias = " + this.qb.getAlias());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getAliases = "
                         + this.qb.getAliases());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getIsQuery = "
                         + this.qb.getIsQuery());
        LOG.info(
                LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getMetaData.aliasToTable = " + this.qb
                        .getMetaData()
                        .getAliasToTable());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getMetaData.NameToDestPartition = "
                         + this.qb.getMetaData().getNameToDestPartition());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getMetaData.NameToDestTable = "
                         + this.qb.getMetaData().getNameToDestTable());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getNumGbys = "
                         + this.qb.getNumGbys());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getNumJoins = "
                         + this.qb.getNumJoins());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getNumSelDi = "
                         + this.qb.getNumSelDi());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getNumSels = "
                         + this.qb.getNumSels());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getQbJoinTree = "
                         + this.qb.getQbJoinTree());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) QueryBlock.getSubqAliases = "
                         + this.qb.getSubqAliases());
        LOG.info(LOG_GY_PREFIX + " -\t\t SemanticAnalyzer.init(boolean) clearPartsCache = " + clearPartsCache);
    }

    boolean analyzeCreateTable(ASTNode child) throws SemanticException {
        if (ast.getToken().getType() == HiveParser.TOK_CREATETABLE) {
            // if it is not CTAS, we don't need to go further and just return
            return (child = analyzeCreateTable(ast, qb, null)) == null;
        } else {
    ...
    public void analyzeInternal(ASTNode ast) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + " \t 1 SemanticAnalyzer.analyzeInternal(ASTNode) ast = " + ast);
        analyzeInternal(ast, new PlannerContext());
        LOG.info(LOG_GY_PREFIX + " \t 2 SemanticAnalyzer.analyzeInternal(ASTNode) ast = " + ast);
    }
    ...
    static class PlannerContext {
        ...
        void setParseTreeAttr(ASTNode child,
                              Phase1Ctx ctx_1) {
            this.child = child;
            this.ctx_1 = ctx_1;
        }
        ...
        void setInsertToken(ASTNode ast,
                            boolean isTmpFileDest) {
        }
    ...
    private static void walkASTMarkTABREF(TableMask tableMask,
                                          ASTNode ast,
                                          Set<String> cteAlias,
                                          Context ctx,
                                          Hive db,
            ...
            if (astNode.getToken().getType() == HiveParser.TOK_TABREF) {
                ...
                for (int index = 1; index < astNode.getChildCount(); index++) {
                    ...
                    if (ct.getToken().getType() == HiveParser.TOK_TABLEBUCKETSAMPLE
                        ...
                        additionalTabInfo.append(ctx.getTokenRewriteStream()
                                                         .toString(ct.getTokenStartIndex(), ct.getTokenStopIndex()));
                    } else {
                        aliasIndex = index;
                    }
                }
    ...
    // unparseTranslator.
    protected static ASTNode rewriteASTWithMaskAndFilter(TableMask tableMask,
                                                         ASTNode ast,
                                                         TokenRewriteStream tokenRewriteStream,
                                                         Context ctx,
                                                         Hive db,
                                                         Map<String, Table> tabNameToTabObject,
                                                         Set<Integer> ignoredTokens) throws SemanticException {
        ...
        if (ast.getChildCount() > 0 && HiveParser.TOK_CTE == ((ASTNode) ast.getChild(0)).getToken().getType()) {
            ...
            for (int index = cte.getChildCount() - 1; index >= 0; index--) {
                ...
                if (cteAlias.contains(alias)) {
                    ...
                    cteAlias.add(alias);
                    walkASTMarkTABREF(tableMask, subq, cteAlias, ctx, db, tabNameToTabObject, ignoredTokens);
                }
            }
            // walk the other part of ast
            for (int index = 1; index < ast.getChildCount(); index++) {
                walkASTMarkTABREF(tableMask,
                                  (ASTNode) ast.getChild(index),
                                  cteAlias,
                                  ctx,
                                  db,
                                  tabNameToTabObject,
        ...
        // there is no CTE, walk the whole AST
            ...
            walkASTMarkTABREF(tableMask, ast, cteAlias, ctx, db, tabNameToTabObject, ignoredTokens);
        }
    ...
    boolean genResolvedParseTree(ASTNode ast,
                                 PlannerContext plannerCtx) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " genResolvedParseTree(ASTNode,PlannerContext) ");
        ASTNode child = ast;
        this.ast = ast;
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 genResolvedParseTree(ASTNode,PlannerContext) token-text = " + ast.getToken()
                .getText());
        // 2. analyze create table command
        // 分析 建表 命令
        if (ast.getToken().getType() == HiveParser.TOK_CREATETABLE) {
            LOG.info(LOG_GY_PREFIX
                             + " \t 2 genResolvedParseTree(ASTNode,PlannerContext) [ TOK_CREATETABLE ] token-text = "
            ...
            LOG.info(LOG_GY_PREFIX + " \t 3 genResolvedParseTree(ASTNode,PlannerContext) [ QUERY ] token-text = "
                             + ast.getToken().getText());
            queryState.setCommandType(HiveOperation.QUERY);
        }
        ...
        if (ast.getToken().getType() == HiveParser.TOK_CREATEVIEW
            ...
            LOG.info(LOG_GY_PREFIX
                             + " \t 4 genResolvedParseTree(ASTNode,PlannerContext) [ TOK_CREATEVIEW | TOK_CREATE_MATERIALIZED_VIEW | TOK_ALTERVIEW | TOK_QUERY ]token-text = "
                             + ast.getToken().getText());
        ...
        switch (ast.getToken().getType()) {
            case HiveParser.TOK_SET_AUTOCOMMIT:
                LOG.info(LOG_GY_PREFIX
                                 + " \t 5 genResolvedParseTree(ASTNode,PlannerContext) [ TOK_SET_AUTOCOMMIT ] token-text = "
                                 + ast.getToken().getText());
                assert ast.getChildCount() == 1;
            ...
            case HiveParser.TOK_ROLLBACK:
                LOG.info(LOG_GY_PREFIX
                                 + " \t 6 genResolvedParseTree(ASTNode,PlannerContext) [ TOK_START_TRANSACTION | TOK_COMMIT | TOK_ROLLBACK ] token-text = "
                                 + ast.getToken().getText());
                if (!(conf.getBoolVar(ConfVars.HIVE_IN_TEST) || conf.getBoolVar(ConfVars.HIVE_IN_TEZ_TEST))) {
                    throw new IllegalStateException(
                            SemanticAnalyzerFactory.getOperation(ast.getToken().getType()) + " is not supported yet.");
                }
        ...
        // 根据 子ASTNode 继续分析
        Phase1Ctx ctx_1 = initPhase1Ctx();
        LOG.info(LOG_GY_PREFIX + " \t 7 genResolvedParseTree(ASTNode,PlannerContext) Phase1Ctx.dest = " + ctx_1.dest);

        try {
            LOG.info(LOG_GY_PREFIX + " \t 9 preProcessForInsert(ASTNode,PlannerContext) getAllDatabases = "
                             + db.getAllDatabases());
            LOG.info(LOG_GY_PREFIX + " \t 9 preProcessForInsert(ASTNode,PlannerContext) getAllTables = "
                             + db.getAllTables());
            LOG.info(LOG_GY_PREFIX + " \t 9 preProcessForInsert(ASTNode,PlannerContext) getDatabaseCurrent = "
        ...
        LOG.info(LOG_GY_PREFIX + " \t 10 Completed phase 1 of Semantic Analysis. ------->");
        ...
        LOG.info(LOG_GY_PREFIX + " \t 11 Completed getting MetaData in Semantic Analysis.  ------->");

        plannerCtx.setParseTreeAttr(child, ctx_1);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " genResolvedParseTree(ASTNode,PlannerContext) ");
        return true;
    }
     ...
     * <br/>
     * 此方法将遍历  INSERT 语句的 AST树，并且 装配一个位于HDFS加密区域的目标表的列表。
     * 这需要确保从select语句中生成的insert语句存储安全
     * See also {@link #genValuesTempTable(ASTNode, QB)}
     *
     * @throws SemanticException
     */
    private void preProcessForInsert(ASTNode node,
                                     QB qb) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " preProcessForInsert(ASTNode,QB) ");
        try {
            if (!(node != null && node.getToken() != null && node.getToken().getType() == HiveParser.TOK_QUERY)) {
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... 1 qb.getSubqAliases = "
                                 + qb.getSubqAliases());
                LOG.info(
                        LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... 2 qb.getAliases = " + qb.getAliases());
                LOG.info(
                        LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... 3 qb.getNumSels = " + qb.getNumSels());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... 4 qb.getNumSelDi = "
                                 + qb.getNumSelDi());
                LOG.info(
                        LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... 5 qb.getNumGbys = " + qb.getNumGbys());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... 6 qb.getTabAliases = "
                                 + qb.getTabAliases());
                return;
            }
            ...
            for (Node child : node.getChildren()) {
                ...
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... dbTab = " + dbTab[0] + ", 1 = "
                                 + dbTab[1]);
                Table t = db.getTable(dbTab[0], dbTab[1]);
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getCompleteName = "
                                 + t.getCompleteName());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getDbName = " + t.getDbName());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getOwner = " + t.getOwner());
                LOG.info(
                        LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getTableName = " + t.getTableName());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getCols = " + t.getCols()
                        .toString());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getAllCols = " + t.getAllCols()
                        .toString());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getInputFormatClass = "
                                 + t.getInputFormatClass());
                LOG.info(
                        LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getTableType = " + t.getTableType());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getPath = " + t.getPath());
                LOG.info(LOG_GY_PREFIX + " \t preProcessForInsert(ASTNode,QB) ... t.getPartitionKeys = "
                                 + t.getPartitionKeys());
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " preProcessForInsert(ASTNode,QB) ");
    }

    Operator genOPTree(ASTNode ast,
                       PlannerContext plannerCtx) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " SemanticAnalyzer.genOPTree(ASTNode,PlannerContext) ");
        Operator op = genPlan(qb);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " SemanticAnalyzer.genOPTree(ASTNode,PlannerContext) ");
        return op;
    }
    ...
    void analyzeInternal(ASTNode ast,
        ...
        // -4 物理执行计划的切分
        // -5 物理执行计划的优化
        // -6 以及mr任务的生成全部都在这1万多行的代码里边的逻辑中
        // 1. Generate Resolved Parse tree from syntax tree
        // 1. 根据语法树 生成 解析树
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) -----> Starting Semantic Analysis ");
        ...
        // 2. 从解析树 生成 操作树
        LOG.info(LOG_GY_PREFIX + " \t 3 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) ");
        // 这里调用的是子类 CalcitePlanner 的genOPTree方法.
        Operator sinkOp = genOPTree(ast, plannerCtx);

        if (!unparseTranslator.isEnabled() && tableMask.isEnabled()) {
            LOG.info(LOG_GY_PREFIX + " \t 4 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) src-tree = "
                             + ast.toStringTree());
            // Here we rewrite the * and also the masking table
            ASTNode tree = rewriteASTWithMaskAndFilter(tableMask,
                                                       ast,
                                                       ...
                                                       tabNameToTabObject,
                                                       ignoredTokens);
            LOG.info(LOG_GY_PREFIX + " \t 4 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) result-tree = "
                             + tree.toStringTree());
            if (tree != ast) {
        ...
        if (createVwDesc != null && !this.ctx.isCboSucceeded()) {
            LOG.info(LOG_GY_PREFIX + " \t 5 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) ");
            ...
            // succeeds.
            LOG.info(LOG_GY_PREFIX + " \t 6 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) ");
        ...
        LOG.info(LOG_GY_PREFIX + " \t 7-Begin SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) resultSechema ");
        Iterator<FieldSchema> it = resultSchema.iterator();
        while (it.hasNext()) {
            FieldSchema fs = it.next();
            LOG.info(LOG_GY_PREFIX
                             + " \t \t -SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) FieldSchema(name = "
                             + fs.getName() + ", type=" + fs.getType() + "， comment=" + fs.getComment() + " ) ");
        }

        LOG.info(LOG_GY_PREFIX + " \t 7-End SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) resultSechema ");

        Set<Entry<String, Table>> tableMapping = tabNameToTabObject.entrySet();
        for (Entry<String, Table> e : tableMapping) {
            LOG.info(
                    LOG_GY_PREFIX + " \t SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext) table = " + e.getKey()
                            + ", table = " + e.getValue().getFields());
        }

        // 4. Generate Parse Context for Optimizer & Physical compiler
        copyInfoToQueryProperties(queryProperties);
        ParseContext pCtx = new ParseContext(queryState,
                                             opToPartPruner,
                                             opToPartList,
                                             topOps,
                                             new HashSet<JoinOperator>(joinContext.keySet()),
        ...
        if (createVwDesc != null) {
            LOG.info(LOG_GY_PREFIX + " \t 8 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext)");
            ...
            if (!createVwDesc.isMaterialized()) {
                ...
                Set<String> postExecHooks = Sets.newHashSet(Splitter.on(",")
                                                                    .trimResults()
                                                                    .omitEmptyStrings()
        ...
        LOG.info(LOG_GY_PREFIX + " \t 9 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext). Optimizer. ");
        ...
        // 如果需要生成列访问状态 - 在列裁剪优化之前一直等待
        boolean isColumnInfoNeedForAuth = SessionState.get().isAuthorizationModeV2() && HiveConf.getBoolVar(conf,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 SemanticAnalyzer.analyzeInternal(ASTNode,PlannerContext). ----->  Completed plan generation.");
    ...
    private void putAccessedColumnsToReadEntity(HashSet<ReadEntity> inputs,
    ...
    private void enforceScanLimits(ParseContext pCtx,
                                   FetchTask fTask) throws SemanticException {
            ...
            if (fTask != null) {
                ...
                if (!fTask.getWork().isNotPartitioned() && fTask.getWork().getLimit() == -1
                    ...
                    throw new SemanticException(ErrorMsg.PARTITION_SCAN_LIMIT_EXCEEDED,
                                                ...
                                                "" + fTask.getWork().getTblDesc().getTableName(),
                                                "" + scanLimit);
                }
            } else {
    ...
    protected void saveViewDefinition() throws SemanticException {
        ...
        if (createVwDesc.getViewOriginalText() == null) {
            String originalText = ctx.getTokenRewriteStream()
                    .toString(viewSelect.getTokenStartIndex(), viewSelect.getTokenStopIndex());
        ...
        String expandedText = ctx.getTokenRewriteStream()
                .toString(viewSelect.getTokenStartIndex(), viewSelect.getTokenStopIndex());
        ...
        if (createVwDesc.getPartColNames() != null) {
            ...
            List<FieldSchema> partitionColumns = derivedSchema.subList(derivedSchema.size() - partColNames.size(),
                                                                       derivedSchema.size());
     ...
     */
    private ExprNodeDesc getExprNodeDescCached(ASTNode expr,
                                               RowResolver input) throws SemanticException {
     ...
     * @param tcCtx Customized type-checking context
     *
    ...
    public Map<ASTNode, ExprNodeDesc> genAllExprNodeDesc(ASTNode expr,
        ...
        for (Map.Entry<ASTNode, ExprNodeDesc> entry : nodeOutputs.entrySet()) {
            ...
            if (tmp[0] != null && columnDesc.getTabAlias() != null && !tmp[0].equals(columnDesc.getTabAlias())
    ...
    private void validate(Task<? extends Serializable> task,
                          boolean reworkMapredWork) throws SemanticException {
     ...
     */
    ASTNode analyzeCreateTable(ASTNode ast,
                               QB qb,
                               PlannerContext plannerCtx) throws SemanticException {

        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 analyzeCreateTable(ASTNode, QB, PlannerContext) >>>>>>>>> ");
        ...
        LOG.info(LOG_GY_PREFIX + " \t 0 analyzeCreateTable(ASTNode, QB, PlannerContext). Creating table " + dbDotTab + " position=" + ast.getCharPositionInLine());
        int numCh = ast.getChildCount();

        LOG.info(LOG_GY_PREFIX + " \t 1 analyzeCreateTable(ASTNode, QB, PlannerContext) numCh = " + numCh);
        LOG.info(LOG_GY_PREFIX + " \t 2 analyzeCreateTable(ASTNode, QB, PlannerContext) ast = " + ast.toStringTree().toString());
        LOG.info(LOG_GY_PREFIX + " \t 3 analyzeCreateTable(ASTNode, QB, PlannerContext) dbDotTab = " + dbDotTab);
        ...
        LOG.info(LOG_GY_PREFIX + " \t 4 analyzeCreateTable(ASTNode, QB, PlannerContext) ifNotExists = " + ifNotExists);
        ...
        switch (command_type) {
            ...
            case CREATE_TABLE: // REGULAR CREATE TABLE DDL
                ...
                LOG.info(LOG_GY_PREFIX + " \t 5 analyzeCreateTable(ASTNode, QB, PlannerContext) table = CREATE_TABLE ");

                CreateTableDesc crtTblDesc = new CreateTableDesc(dbDotTab,
                                                                 isExt,
                                                                 isTemporary,
                                                                 cols,
                                                                 partCols,
            ...
            case CTLT: // create table like <tbl_name>
                 ...
                 */
                tblProps = addDefaultProperties(tblProps);
                LOG.info(LOG_GY_PREFIX + " \t 6 analyzeCreateTable(ASTNode, QB, PlannerContext) table = CTLT ");
                if (isTemporary) {
            ...
            case CTAS: // create table as select
                ...
                LOG.info(LOG_GY_PREFIX + " \t 7 analyzeCreateTable(ASTNode, QB, PlannerContext) table = CTAS ");
                /**
                 * TODO lipeng
                ...
                if (isTemporary) {
                    if (!ctx.isExplainSkipExecution() && !isMaterialization) {
                        ...
                        if (ss == null) {
                            throw new SemanticException(
                                    "No current SessionState, cannot create temporary table " + dbName + "." + tblName);
                        }
                        ...
                        if (tables != null && tables.containsKey(tblName)) {
                            throw new SemanticException(
                                    "Temporary table " + dbName + "." + tblName + " already exists");
                        }
                    }
                } else {
                ...
                tableDesc = new CreateTableDesc(qualifiedTabName[0],
                                                dbDotTab,
                                                isExt,
                                                isTemporary,
                                                cols,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 analyzeCreateTable(ASTNode, QB, PlannerContext) <<<<<<<<< ");
        return null;
    }

    private void addDbAndTabToOutputs(String[] qualifiedTabName,
                                      TableType type) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 addDbAndTabToOutputs(qualifiedTabName, TableType) >>>>>>>>>> ");
        LOG.info(LOG_GY_PREFIX + " \t 0 addDbAndTabToOutputs(qualifiedTabName, TableType) qualifiedTabName = " + qualifiedTabName);
        LOG.info(LOG_GY_PREFIX + " \t 1 addDbAndTabToOutputs(qualifiedTabName, TableType) qualifiedTabName = " + qualifiedTabName[0]);
        Database database = getDatabase(qualifiedTabName[0]);
        LOG.info(LOG_GY_PREFIX + " \t 2 addDbAndTabToOutputs(qualifiedTabName, TableType) database = " + database);
        outputs.add(new WriteEntity(database, WriteEntity.WriteType.DDL_SHARED));

        Table t = new Table(qualifiedTabName[0], qualifiedTabName[1]);
        t.setTableType(type);

        LOG.info(LOG_GY_PREFIX + " \t 3 addDbAndTabToOutputs(qualifiedTabName, TableType) table = " + t.getTableName());
        LOG.info(LOG_GY_PREFIX + " \t 4 addDbAndTabToOutputs(qualifiedTabName, TableType) table.type = " + t.getTableType());
        outputs.add(new WriteEntity(t, WriteEntity.WriteType.DDL_NO_LOCK));
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 addDbAndTabToOutputs(ASTNode, TableType) <<<<<<<<< ");
    }

    protected ASTNode analyzeCreateView(ASTNode ast,
                                        QB qb,
                                        PlannerContext plannerCtx) throws SemanticException {
        ...
        if (isMaterialized) {
            createVwDesc = new CreateViewDesc(dbDotTable,
                                              cols,
                                              comment,
                                              tblProps,
                                              partColNames,
    ...
    private void validateCreateView() throws SemanticException {
        try {
            ...
            if (createVwDesc.getOrReplace() && oldView != null) {
                ...
                if (!oldView.getTableType().equals(TableType.VIRTUAL_VIEW) && !oldView.getTableType()
                        .equals(TableType.MATERIALIZED_VIEW)) {
                    String tableNotViewErrorMsg =
                            "The following is an existing table, not a view: " + createVwDesc.getViewName();
                ...
                if (!createVwDesc.isMaterialized()) {
                    ...
                    String partitionViewErrorMsg =
                            "The following view has partition, it could not be replaced: " + createVwDesc.getViewName();
                    try {
                        if ((createVwDesc.getPartCols() == null || createVwDesc.getPartCols().isEmpty() || !createVwDesc
                                .getPartCols()
                                .equals(oldView.getPartCols())) && !oldView.getPartCols().isEmpty()
                                && !db.getPartitions(oldView).isEmpty()) {
                            throw new SemanticException(ErrorMsg.REPLACE_VIEW_WITH_PARTITION.getMsg(
                                    partitionViewErrorMsg));
                        }
                    } catch (HiveException e) {
    ...
    public void processPositionAlias(ASTNode ast) throws SemanticException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 processPositionAlias(ASTNode) ");
        ...
        LOG.info(LOG_GY_PREFIX + " \t 1 processPositionAlias(ASTNode) isBothByPos = " + isBothByPos);
        LOG.info(LOG_GY_PREFIX + " \t 2 processPositionAlias(ASTNode) isGbyByPos = " + isGbyByPos);
        LOG.info(LOG_GY_PREFIX + " \t 3 processPositionAlias(ASTNode) isObyByPos = " + isObyByPos);
...
//            LOG.info(LOG_GY_PREFIX + " \t  ASTNode --------------->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ");
            // 一直循环，直到找到 select, groupby, orderby节点.
////            LOG.info(LOG_GY_PREFIX + " \t processPositionAlias(ASTNode) name = " + next.getName() + ", text="
//                             + next.getText() + ", string-tree = " + next.toStringTree());
            ...
            for (int child_pos = 0; child_pos < child_count; ++child_pos) {
                ...
                if (type == HiveParser.TOK_SELECT) {
                    LOG.info(LOG_GY_PREFIX + " \t\t\t processPositionAlias(ASTNode) 1 select-node = " + next.getName());
                    selectNode = node;
                } else if (type == HiveParser.TOK_GROUPBY) {
                    LOG.info(LOG_GY_PREFIX + " \t\t\t processPositionAlias(ASTNode) 2 groupby-node = " + next.getName());
                    groupbyNode = node;
                } else if (type == HiveParser.TOK_ORDERBY) {
                    LOG.info(LOG_GY_PREFIX + " \t\t\t processPositionAlias(ASTNode) 3 orderby-node = " + next.getName());
                    orderbyNode = node;
                }
            }
                        ...
                        if (node.getToken().getType() == HiveParser.Number) {
                            if (isGbyByPos) {
                                ...
                                if (pos > 0 && pos <= selectExpCnt) {
                                    LOG.info(LOG_GY_PREFIX + " 11 \t\t\t processPositionAlias(ASTNode) pos = " + pos
                                                     + ", node-text=" + node.toStringTree() + ", groupby-node = "
                                                     + groupbyNode.getChild(child_pos).toStringTree() + ", select-node="
                                                     + selectNode.getChild(pos - 1).toStringTree());
                                    groupbyNode.setChild(child_pos, selectNode.getChild(pos - 1).getChild(0));
                                } else {
                                    throw new SemanticException(ErrorMsg.INVALID_POSITION_ALIAS_IN_GROUPBY.getMsg(
                        ...
                        if (node != null && node.getToken().getType() == HiveParser.Number) {
                            if (isObyByPos) {
                                if (!isAllCol) {
                                    int pos = Integer.parseInt(node.getText());
                                    if (pos > 0 && pos <= selectExpCnt) {
                                        LOG.info(LOG_GY_PREFIX + " 22 \t\t\t processPositionAlias(ASTNode) pos = " + pos
                                                         + ", node-text=" + node.toStringTree() + ", colNode = "
                                                         + colNode.toStringTree() + ", select-node=" + selectNode.getChild(
                                                pos - 1).toStringTree());
        ...
        Iterator<ASTNode> it = stack.iterator();
        while (it.hasNext()) {
            ASTNode node = it.next();
            LOG.info(LOG_GY_PREFIX + " \t node = " + node.toStringTree());
        }
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 processPositionAlias(ASTNode) ");
    }
     ...
     */
    private PTFInputSpec processPTFSource(QB qb,
                                          ASTNode inputNode) throws SemanticException {
     ...
     *   PTF invocation.
     */
    private PartitionedTableFunctionSpec processPTFChain(QB qb,
                                                         ASTNode ptf) throws SemanticException {
    ...
    private void handleQueryWindowClauses(QB qb,
                                          Phase1Ctx ctx_1,
    ...
    private WindowFunctionSpec processWindowFunction(ASTNode node,
    ...
    private boolean containsLeadLagUDF(ASTNode expressionTree) {
        ...
        if (exprTokenType == HiveParser.TOK_FUNCTION) {
            ...
            if (expressionTree.getChild(0).getType() == HiveParser.Identifier) {
                ...
                if (FunctionRegistry.LAG_FUNC_NAME.equals(functionName) || FunctionRegistry.LEAD_FUNC_NAME.equals(
                        functionName)) {
                    return true;
                }
            }
    ...
    private void processQueryWindowClause(WindowingSpec spec,
                                          ASTNode node) throws SemanticException {
        ASTNode nameNode = (ASTNode) node.getChild(0);
        ...
        if (spec.getWindowSpecs() != null && spec.getWindowSpecs().containsKey(nameNode.getText())) {
            throw new SemanticException(generateErrorMessage(nameNode,
                                                             "Duplicate definition of window " + nameNode.getText() + " is not allowed"));
        }
        WindowSpec ws = processWindowSpec(wsNode);
    ...
    private WindowFrameSpec processWindowFrame(ASTNode node) throws SemanticException {
        ...
        return new WindowFrameSpec(type == HiveParser.TOK_WINDOWVALUES ? WindowType.RANGE : WindowType.ROWS,
                                   start,
                                   end);
    }

    private BoundarySpec processBoundary(int frameType,
                                         ASTNode node) throws SemanticException {
     ...
     *
     */
    private boolean isValidGroupBySelectList(QB currQB,
                                             String clause) {
        ConstantExprCheck constantExprCheck = new ConstantExprCheck();
        AggregationExprCheck aggrExprCheck = new AggregationExprCheck(currQB.getParseInfo()
                                                                              .getAggregationExprsForClause(clause));
    ...
    private PTFDesc translatePTFInvocationSpec(PTFInvocationSpec ptfQSpec,
                                               RowResolver inputRR)
            throws SemanticException {
    ...
    Operator genPTFPlan(PTFInvocationSpec ptfQSpec,
                        Operator input) throws SemanticException {
    ...
    Operator genWindowingPlan(QB qb,
                              WindowingSpec wSpec,
                              Operator input) throws SemanticException {
        ...
        if (!isCBOExecuted() && !qb.getParseInfo().getDestToGroupBy().isEmpty()) {
            ...
            for (WindowExpressionSpec wExprSpec : wSpec.getWindowExpressions()) {
                // Special handling of grouping function
                wExprSpec.setExpression(rewriteGroupingFunctionAST(getGroupByForClause(qb.getParseInfo(),
                                                                                       selClauseName), wExprSpec.getExpression(), !cubeRollupGrpSetPresent));
            }
        }
    ...
    private Operator genReduceSinkPlanForWindowing(WindowingSpec spec,
        ...
        return genReduceSinkPlan(input,
                                 partCols,
                                 orderCols,
                                 order.toString(),
                                 nullOrder.toString(),
    ...
    public static ArrayList<WindowExpressionSpec> parseSelect(String selectExprStr) throws SemanticException {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (selExpr.getType() != HiveParser.TOK_SELEXPR) {
                throw new SemanticException(String.format("Only Select expressions supported in dynamic select list: %s",
                                                          selectExprStr));
            }
            ASTNode expr = (ASTNode) selExpr.getChild(0);
            if (expr.getType() == HiveParser.TOK_ALLCOLREF) {
                throw new SemanticException(String.format("'%s' column not allowed in dynamic select list",
                                                          selectExprStr));
            }
            ASTNode aliasNode = selExpr.getChildCount() > 1 && selExpr.getChild(1).getType()
                    == HiveParser.Identifier ? (ASTNode) selExpr.getChild(1) : null;
            String alias = null;
            if (aliasNode != null) {
    ...
    private void addAlternateGByKeyMappings(ASTNode gByExpr,
                                            ColumnInfo colInfo,
                                            Operator<? extends OperatorDesc> reduceSinkOp,
                                            RowResolver gByRR) {
        if (gByExpr.getType() == HiveParser.DOT && gByExpr.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL) {
            String tab_alias = BaseSemanticAnalyzer.unescapeIdentifier(gByExpr.getChild(0)
                                                                               .getChild(0)
                                                                               .getText()
                                                                               .toLowerCase());
    ...
    private WriteEntity.WriteType determineWriteType(LoadTableDesc ltd,
    ...
    private WriteEntity.WriteType getWriteType(String dest) {
        return updating(dest) ? WriteEntity.WriteType.UPDATE : (deleting(dest) ? WriteEntity.WriteType.DELETE : WriteEntity.WriteType.INSERT);
    }
    ...
    private AcidUtils.Operation getAcidType(String destination) {
        return deleting(destination) ? AcidUtils.Operation.DELETE : (updating(destination) ? AcidUtils.Operation.UPDATE : AcidUtils.Operation.INSERT);
    }
    ...
    private void copyInfoToQueryProperties(QueryProperties queryProperties) {
        if (qb != null) {
            ...
            queryProperties.setHasOuterOrderBy(
                    !qb.getParseInfo().getIsSubQ() && !qb.getParseInfo().getDestToOrderBy().isEmpty());

...
public class Context {
     ...
     */
    public DestClausePrefix addDestNamePrefix(int pos,
                                              DestClausePrefix prefix) {
     ...
     */
    private Path getScratchDir(String scheme,
                               String authority,
                               boolean mkdir,
                               String scratchDir) {
    ...
    public DataInput getStream() {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 getStream() ");
        try {
            if (!initialized) {
                initialized = true;
                if ((resFile == null) && (resDir == null)) {
                    LOG.info(LOG_GY_PREFIX + " \t 0 getStream() resFile == null && resDir == null");
                    LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999-initialized=" + initialized + ", resFile-resDir-null getStream() ");
                    return null;
                }


                if (resFile != null) {
                    LOG.info(LOG_GY_PREFIX + " \t 1 getStream() resFile.parent = " + resFile.getParent().toString());
                    LOG.info(LOG_GY_PREFIX + " \t 2 getStream() resFile.name = " + resFile.getName());
                    LOG.info(LOG_GY_PREFIX + " \t 3 getStream() resFile.toUri = " + resFile.toUri().getPath());
                    LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999-initialized=" + initialized + ", resFile-not-null getStream() ");
                    return resFile.getFileSystem(conf).open(resFile);
                }

                resFs = resDir.getFileSystem(conf);
                LOG.info(LOG_GY_PREFIX + " \t 4 getStream() resDir != null. resFile = " + resFile.getName());
                ...
                if (pos == 0) {
                    LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999-initialized=" + initialized + ", pos-0 getStream() ");
                    return null;
                }
                LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999-initialized=" + initialized + ", resFs getStream() ");
                return resFs.open(resDirPaths[resDirFilesNum++]);
            } else {
                LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999-initialized=" + initialized + ", getStream() ");
    ...
    private DataInput getNextStream() {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 getNextStream() ");
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 getNextStream() ");
        return null;
    }
     ...
     */
    private static boolean strEquals(String str1,
                                     String str2) {
    ...
    public void addMaterializedTable(String cteName,
                                     Table table) {

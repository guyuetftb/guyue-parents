...
public abstract class Task<T extends Serializable> implements Serializable, Node {
    ...
    public int executeTask() {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 000 Task.executeTask() ");
        try {
            ...
            this.setDone();
            LOG.info(LOG_GY_PREFIX + " \t 4 status = " + this.taskState);
            if (ss != null) {
                ss.getHiveHistory().logPlanProgress(queryPlan);
            }
            LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 999 Task.executeTask() ");
            return retval;
        } catch (IOException e) {
    ...
    protected void pushFeed(FeedType feedType,
                            Object feedValue) {
    ...
    protected void receiveFeed(FeedType feedType,
                               Object feedValue) {
    }

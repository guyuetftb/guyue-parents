...
public class TezSessionState {
    ...
    public TezSessionState(DagUtils utils) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 2 TezSessionState(DagUtils) ");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 2 TezSessionState(DagUtils) ");
        this.utils = utils;
    }
    ...
    public TezSessionState(String sessionId) {
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 1 TezSessionState(String) ");
        this.sessionId = sessionId;
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 1 TezSessionState(String) sessionId = " + sessionId);
    }
    ...
    public void open(HiveConf conf)
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " open(HiveConf) ");
        Set<String> noFiles = null;

        open(conf, noFiles, null);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " open(HiveConf) ");
    }
    ...
    protected void openInternal(final HiveConf conf, Collection<String> additionalFiles,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  ");
        this.conf = conf;
        ...
        LOG.info(LOG_GY_PREFIX + "\t 1 openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  additionalFiles = " + additionalFiles);
        LOG.info(LOG_GY_PREFIX + "\t 2  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  isAsync = " + isAsync);
        LOG.info(LOG_GY_PREFIX + "\t 3  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  console = " + console);
        LOG.info(LOG_GY_PREFIX + "\t 4  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  scratchDir = " + scratchDir);
        this.queueName = confQueueName;
        this.doAsEnabled = conf.getBoolVar(HiveConf.ConfVars.HIVE_SERVER2_ENABLE_DOAS);
        LOG.info(LOG_GY_PREFIX + "\t 5  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  queueName = " + queueName);
        ...
        LOG.info(LOG_GY_PREFIX + "\t 6  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  sessionId = " + sessionId + ", user = " + user);

        // create the tez tmp dir
        tezScratchDir = scratchDir == null ? createTezDir(sessionId) : scratchDir;

        LOG.info(LOG_GY_PREFIX + "\t 7  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  tezScratchDir = " + tezScratchDir);
        ...
        LOG.info(LOG_GY_PREFIX + "\t 8  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  ExecJarPathLocal = " + utils.getExecJarPathLocal());
        ...
        LOG.info(LOG_GY_PREFIX + "\t 9  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  utils.getBaseName(appJarLr) = " + utils.getBaseName(appJarLr));
        ...
        LOG.info(LOG_GY_PREFIX + "\t 10  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  TezConfiguration-Created. ");
        ...
        LOG.info(LOG_GY_PREFIX + "\t 11  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  ConfVars.HIVE_PREWARM_ENABLED = "
                + HiveConf.getBoolVar(conf, ConfVars.HIVE_PREWARM_ENABLED));
        if (HiveConf.getBoolVar(conf, ConfVars.HIVE_PREWARM_ENABLED)) {
            ...
            LOG.info(LOG_GY_PREFIX + "\t 11-1  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  ConfVars.HIVE_PREWARM_NUM_CONTAINERS = "
                    + HiveConf.getIntVar(conf, ConfVars.HIVE_PREWARM_NUM_CONTAINERS));

            LOG.info(LOG_GY_PREFIX + "\t 11-2  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  TezConfiguration.TEZ_AM_SESSION_MIN_HELD_CONTAINERS = "
                    + tezConfig.getInt(TezConfiguration.TEZ_AM_SESSION_MIN_HELD_CONTAINERS,
                    TezConfiguration.TEZ_AM_SESSION_MIN_HELD_CONTAINERS_DEFAULT));
        }

        setupSessionAcls(tezConfig, conf);

        LOG.info(LOG_GY_PREFIX + "\t 12  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path) BBBBB-TezClient. ");
        // TODO 2018-05-17 by lipeng.
        final TezClient session = TezClient.newBuilder("HIVE-" + sessionId, tezConfig)
                ...
                .build();
        LOG.info(LOG_GY_PREFIX + "\t 12-1  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path) tezClient.name =  " + session.getClientName());
        LOG.info(LOG_GY_PREFIX + "\t 12-1  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path) tezClient.isSession =  true. ");
        LOG.info(LOG_GY_PREFIX + "\t 13  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path) EEEEE-TezClient. ");
        LOG.info(LOG_GY_PREFIX + "\t Opening new Tez Session (id: " + sessionId + ", scratch dir: " + tezScratchDir + ")");

        TezJobMonitor.initShutdownHook();
        if (!isAsync) {
            LOG.info(LOG_GY_PREFIX + "\t 14  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path) isAsync = " + isAsync);
            startSessionAndContainers(session, conf, commonLocalResources, tezConfig, false);
            this.session = session;
        } else {
            LOG.info(LOG_GY_PREFIX + "\t 15  openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path) isAsync = " + isAsync);
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " openInternal(HiveConf,Collection<String>), boolean,LogHelper, Path)  ");
    }
    ...
    private TezClient startSessionAndContainers(TezClient session, HiveConf conf,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " startSessionAndContainers(TezClient,HiveConf,Map<String, LocalResource>,TezConfiguration,boolean)  ");
        LOG.info(LOG_GY_PREFIX + " 1 startSessionAndContainers(TezClient,HiveConf,Map<String, LocalResource>,TezConfiguration,boolean)  BBBBB-TezClient.start() ");
        session.start();
        ...
        try {
            ...
            try {
                LOG.info(LOG_GY_PREFIX + " 2 startSessionAndContainers(TezClient,HiveConf,Map<String, LocalResource>,TezConfiguration,boolean)  TezClient wait APP-MASTER.  ");
            ...
            LOG.info(LOG_GY_PREFIX + LOG_GY_END + " startSessionAndContainers(TezClient,HiveConf,Map<String, LocalResource>,TezConfiguration,boolean)  ");
        }
    }

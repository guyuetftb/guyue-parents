...
public class TezClient {
    ...
    TezClient(String name, TezConfiguration tezConf,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 1 TezClient(String,TezConfiguration,Map<String, LocalResource>,Credentials)");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 1 TezClient(String,TezConfiguration,Map<String, LocalResource>,Credentials)");
    }

    private TezClient(String name, TezConfiguration tezConf, boolean isSession) {
        this(name, tezConf, isSession, null, null);
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 2 TezClient(String,TezConfiguration,boolean)");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 2 TezClient(String,TezConfiguration,boolean)");
    }
    ...
    protected TezClient(String name, TezConfiguration tezConf, boolean isSession,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 3 TezClient(String,TezConfiguration,boolean,Map<String, LocalResource>,Credentials)");
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 3 TezClient(String,TezConfiguration,boolean,Map<String, LocalResource>,Credentials)");
    }
    ...
    protected TezClient(String name, TezConfiguration tezConf, boolean isSession,
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 4 TezClient(String,TezConfiguration,boolean,Map<String, LocalResource>,Credentials)");

        this.clientName = name;
        this.isSession = isSession;

        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " \t 0 TezClient(String,TezConfiguration,boolean,Map<String, LocalResource>,Credentials) clientName = " + clientName);
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " \t 1 TezClient(String,TezConfiguration,boolean,Map<String, LocalResource>,Credentials) isSession = " + isSession);
        ...
        LOG.info(LOG_GY_PREFIX + " \t Tez Client Version: " + apiVersionInfo.toString());
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 4 TezClient(String,TezConfiguration,boolean,Map<String, LocalResource>,Credentials)");
    }
    ...
    public static TezClient create(String name, TezConfiguration tezConf) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " 1 create(String,TezConfiguration)");
    ...
    public static TezClient create(String name, TezConfiguration tezConf,
                                   ...
                                   @Nullable Credentials credentials) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 2 TezClient(String,TezConfiguration,Map<String, LocalResource>,Credentials)");
    ...
    public static TezClient create(String name, TezConfiguration tezConf, boolean isSession) {
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " 3 TezClient(String,TezConfiguration,boolean)");
    ...
    public synchronized void start() throws TezException, IOException {
        ...
        if (this.amConfig.getTezConfiguration().getBoolean(
            ...
            LOG.info(LOG_GY_PREFIX + " \t 1 start()");
            ...
            if (!javaOptsCheckerClassName.isEmpty()) {
                try {
                    LOG.info(LOG_GY_PREFIX + " \t 2 start()");
                ...
                LOG.info(LOG_GY_PREFIX + " \t 3 start()");
        ...
        if (isSession) {
            LOG.info(LOG_GY_PREFIX + " \t 4 start() Session mode. Starting session.");
            ...
            LOG.info(LOG_GY_PREFIX + " \t 4-1 start() clientTimeout = " + clientTimeout);

            try {
                if (sessionAppId == null) {
                    LOG.info(LOG_GY_PREFIX + " \t 5 start() sessionAppId = " + sessionAppId);
                    sessionAppId = createApplication();
                    LOG.info(LOG_GY_PREFIX + " \t 5-1 start() sessionAppId = " + sessionAppId);
                }
                ...
                LOG.info(LOG_GY_PREFIX + " \t 99 start() The url to track the Tez Session: " + appReport.getTrackingUrl());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getApplicationType = " + appReport.getApplicationType());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getHost = " + appReport.getHost());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getDiagnostics = " + appReport.getDiagnostics());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getName = " + appReport.getName());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getQueue = " + appReport.getQueue());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getApplicationId = " + appReport.getApplicationId());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getProgress = " + appReport.getProgress());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getStartTime = " + appReport.getStartTime());
                LOG.info(LOG_GY_PREFIX + " \t 99 start() appReport.getYarnApplicationState.name = " + appReport.getYarnApplicationState().name());
    ...
    public synchronized DAGClient submitDAG(DAG dag) throws TezException, IOException {
        if (isSession) {
            LOG.info(LOG_GY_PREFIX + " \t submitDAG(DAG) action = submitDAGSession");
            return submitDAGSession(dag);
        } else {
            LOG.info(LOG_GY_PREFIX + " \t submitDAG(DAG) action = submitDAGApplication");
    ...
    private DAGClient submitDAGSession(DAG dag) throws TezException, IOException {
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " submitDAGSession(DAG)");
        ...
        LOG.info(LOG_GY_PREFIX + " \t Submitting dag to TezSession"
        ...
        LOG.info(LOG_GY_PREFIX + " \t Submitted dag to TezSession"
                + ", sessionName=" + clientName
                + ", applicationId=" + sessionAppId
                + ", dagId=" + dagId
                + ", dagName=" + dag.getName());
        ...
        DAGClientImpl dagClient = new DAGClientImpl(sessionAppId, dagId,
                amConfig.getTezConfiguration(),
                amConfig.getYarnConfiguration(),
                frameworkClient);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " submitDAGSession(DAG)");
        return dagClient;
    }
    ...
    public synchronized boolean waitTillReady(long timeout, TimeUnit unit)
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " waitTillReady(long,TimeUnit) ");
        timeout = unit.toMillis(timeout);
        if (!isSession) {
            // nothing to wait for in non-session mode
            LOG.info(LOG_GY_PREFIX + " 0 waitTillReady(long,TimeUnit)  BBBBB-TezClient.start() isSession = " + isSession);
            return true;
        }
        ...
        while (true) {
            ...
            if (status.equals(TezAppMasterStatus.SHUTDOWN)) {
                LOG.info(LOG_GY_PREFIX + " 1 waitTillReady(long,TimeUnit) master-status = SHUTDOWN ");
            ...
            if (status.equals(TezAppMasterStatus.READY)) {
                LOG.info(LOG_GY_PREFIX + " 2 waitTillReady(long,TimeUnit) master-status = READY ");
                return true;
            }
            if (timeout == 0) {
                Thread.sleep(SLEEP_FOR_READY);
                LOG.info(LOG_GY_PREFIX + " 3 waitTillReady(long,TimeUnit) master-status = SLEEP_FOR_READY ");
                continue;
            }
            ...
            if (timeLimit > now) {
                ...
                LOG.info(LOG_GY_PREFIX + " 4 waitTillReady(long,TimeUnit) master-status = SLEEP ");
            } else {
                LOG.info(LOG_GY_PREFIX + " 5 waitTillReady(long,TimeUnit) master-status = EXCEPTION ");
                return false;
            }
        }
    }
    ...
    private DAGClient submitDAGApplication(DAG dag)
            throws TezException, IOException {
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " submitDAGApplication(DAG)");
        ApplicationId appId = createApplication();
        LOG.info(LOG_GY_PREFIX + " \t submitDAGApplication(DAG) ApplicationId = " + appId.toString());
        DAGClient dagClient = submitDAGApplication(appId, dag);
        LOG.info(LOG_GY_PREFIX + LOG_GY_END + " submitDAGApplication(DAG)");
        return dagClient;
    }
    ...
    DAGClient submitDAGApplication(ApplicationId appId, DAG dag)
        ...
        LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " submitDAGApplication(DAG)");
        LOG.info(LOG_GY_PREFIX + " \t submitDAGApplication(DAG). Submitting DAG application with id: " + appId);
        try {
            ...
            LOG.info(LOG_GY_PREFIX + " \t Submitting DAG to YARN"
                    + ", applicationId=" + appId
                    + ", dagName=" + dag.getName()
                    + callerContextStr);
        ...
        DAGClient dagClient = getDAGClient(appId, amConfig.getTezConfiguration(), amConfig.getYarnConfiguration(),
                frameworkClient);
    ...
    private ApplicationId createApplication() throws TezException, IOException {
        ApplicationId appId = null;
        try {
            LOG.info(LOG_GY_PREFIX + LOG_GY_BEGIN + " createApplication() ");
            appId = frameworkClient.createApplication().
                    getNewApplicationResponse().getApplicationId();
            LOG.info(LOG_GY_PREFIX + " \t createApplication() applicationId = " + appId.toString());
            LOG.info(LOG_GY_PREFIX + LOG_GY_END + " createApplication() ");
        } catch (YarnException e) {
            throw new TezException(e);
        }
        return appId;

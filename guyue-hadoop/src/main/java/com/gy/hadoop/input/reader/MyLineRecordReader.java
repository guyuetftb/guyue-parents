...
public class MyLineRecordReader extends RecordReader<LongWritable, Text> {
	...
	public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
		...
		LOG.info("  ========> initialize(). mapreduce.input.linerecordreader.line.maxlength=" + job.get(MAX_LINE_LENGTH));
		LOG.info("  ========> initialize(). start=" + start);
		LOG.info("  ========> initialize(). pos=" + pos);
		LOG.info("  ========> initialize(). end=" + end);
		LOG.info("  ========> initialize(). key=" + key);
		LOG.info("  ========> initialize(). value=" + value);
		this.maxLineLength = job.getInt(MAX_LINE_LENGTH, Integer.MAX_VALUE);
		LOG.info("  ========> initialize(). split=" + split.toString());

		start = split.getStart();
		end = start + split.getLength();
		final Path file = split.getPath();
		LOG.info("  ========> initialize(). split.path=" + file.toUri().toString());
		...
		if (isCompressedInput()) {
			...
			if (codec instanceof SplittableCompressionCodec) {
				...
				if (null == this.recordDelimiterBytes) {
					in = new MyLineReader(codec.createInputStream(fileIn, decompressor), job);
				} else {
					in = new MyLineReader(codec.createInputStream(fileIn, decompressor), job, this.recordDelimiterBytes);
				}
				filePosition = fileIn;
			}
		} else {
	...
	private int maxBytesToConsume(long pos) {
		LOG.info("========>maxBytesToConsume(). Integer.MAX_VALUE=" + Integer.MAX_VALUE);
		LOG.info("========>maxBytesToConsume(). end=" + end);
		LOG.info("========>maxBytesToConsume(). pos=" + pos);
		LOG.info("========>maxBytesToConsume(). end - pos=" + (end - pos));

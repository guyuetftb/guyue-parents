...
public abstract class MyFileInputFormat<K, V> extends InputFormat<K, V> {
    ...
    private static final Log LOG = LogFactory.getLog(MyFileInputFormat.class);
    ...
    public static long getMaxSplitSize(JobContext context) {
        LOG.info("==========->getMaxSplitSize(). mapreduce.input.fileinputformat.split.maxsize = " + context.getConfiguration().get(SPLIT_MAXSIZE));
    ...
    protected List<FileStatus> listStatus(JobContext job) throws IOException {
        ...
        for (Path p : dirs) {
            LOG.info("==========>listStatus(). dirs.length=" + dirs.length + ", path=" + p.toUri().toString());
        }
        ...
        for (FileStatus fs : result) {
            LOG.info("==========>listStatus(). fileStatus.length=" + result.size() + ", fileStatus-Path=" + fs.getPath().toUri().toString());
        }
        return result;
    }
    ...
    public List<InputSplit> getSplits(JobContext job) throws IOException {
        ...
        LOG.info("==========>getSplits(). maxSize=" + maxSize + ", minSize=" + minSize);
        ...
        for (FileStatus file : files) {
            ...
            LOG.info("==========>getSplits(). file=" + file.getPath().toString() + ",length=" + length);
            if (length != 0) {
                FileSystem fs = path.getFileSystem(job.getConfiguration());
                BlockLocation[] blkLocations = fs.getFileBlockLocations(file, 0, length);
                for (BlockLocation bl : blkLocations) {
                    LOG.info("==========>getSplits(). block=" + bl.getNames()[0] + ",block-length:" + bl.getLength());
                }
                if (isSplitable(job, path)) {
                    long blockSize = file.getBlockSize();
                    LOG.info("==========>getSplits(). file=" + file.getPath().toString() + ",block-size:" + blockSize);
                    long splitSize = computeSplitSize(blockSize, minSize, maxSize);
                    LOG.info("==========>getSplits(). splitSize=" + splitSize);

                    long bytesRemaining = length;
                    LOG.info("==========>getSplits(). bytesRemaining=" + bytesRemaining + ", splitSize=" + splitSize + ", value=" + (((double) bytesRemaining) / splitSize));
                    while (((double) bytesRemaining) / splitSize > SPLIT_SLOP) {
                        int blkIndex = getBlockIndex(blkLocations, length - bytesRemaining);
                        LOG.info("==========>getSplits(). while...blkIndex=" + blkIndex);
                    ...
                    if (bytesRemaining != 0) {
                        int blkIndex = getBlockIndex(blkLocations, length - bytesRemaining);
                        LOG.info("==========>getSplits(). blkIndex=" + blkIndex);
        ...
        for (InputSplit is : splits) {
            try {
                LOG.info("==========>getSplits(). splits: fileSplit=" + is.toString());
                for (String l : is.getLocations()) {
                    LOG.info("    ==========> location=" + l);
                }
    ...
    protected int getBlockIndex(BlockLocation[] blkLocations, long offset) {
        for (int i = 0; i < blkLocations.length; i++) {
            ...
            if ((blkLocations[i].getOffset() <= offset) && (offset < blkLocations[i].getOffset() + blkLocations[i].getLength())) {
                LOG.info("==========>getBlockIndex(). offset=" + offset + ",index=" + i + ", block-offset=" + blkLocations[i].getOffset() + ", length="
    ...
    public static void setInputPaths(Job job, Path... inputPaths) throws IOException {
        ...
        for (int i = 1; i < inputPaths.length; i++) {
            ...
            LOG.info("==========>setInputPaths() index=" + i + ", path=" + path.toString());
            str.append(StringUtils.escapeString(path.toString()));
        }
        LOG.info("==========>setInputPaths() final-path=" + str.toString());

...
public abstract class DataJoinReducerBase extends JobBase {
	...
	private SortedMap<Object, ResetableIterator> regroup(Object key, Iterator arg1, Reporter reporter) {
		this.numOfValues = 0;
		SortedMap<Object, ResetableIterator> sortMapTagRecordIt = new TreeMap<Object, ResetableIterator>();
		...
		while (arg1.hasNext()) {
			...
			ResetableIterator recordIt = sortMapTagRecordIt.get(tag);
			if (recordIt == null) {
				recordIt = createResetableIterator();
				sortMapTagRecordIt.put(tag, recordIt);
			}
			recordIt.add(aRecord);
		}
		if (this.numOfValues > this.largestNumOfValues) {
			this.largestNumOfValues = numOfValues;
			LOG.info("----regroup key: " + key.toString() + " this.largestNumOfValues: " + this.largestNumOfValues);
		}
		LOG.info("----regroup key: " + key.toString() + ", size=" + sortMapTagRecordIt.size());
		return sortMapTagRecordIt;
	}
	...
	private void joinAndCollect(Object[] tags, ResetableIterator[] values, int pos, Object[] partialList, Object key, OutputCollector output, Reporter reporter) throws IOException {
		LOG.info("----joinAndCollect key: " + key.toString() + ", tag-length= " + tags.length + ", values-length=" + values.length + ", pos=" + pos);
		if (values.length == pos) {
			...
			LOG.info("----joinAndCollect key: " + key.toString() + ", writting data.");
			return;
		}
	...
	public void map(Object arg0, Object arg1, OutputCollector arg2, Reporter arg3) {

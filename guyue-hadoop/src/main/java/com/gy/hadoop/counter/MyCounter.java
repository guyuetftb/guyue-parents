...
public class MyCounter {
	...
	public static class MyCounterMap extends Mapper<Text, Text, Text, Text> {
		...
		protected void map(Text key, Text value, Context context) throws java.io.IOException, InterruptedException {
			...
			if (null == arr_value || arr_value.length < 2) {
				ct = context.getCounter("gender-counter", "other");
				ct.increment(1);
				return;
			}

			if ("m".equals(arr_value[5])) {
				ct = context.getCounter("gender-counter", "male");
				ct.increment(1);
			} else {
				ct = context.getCounter("gender-counter", "female");
				ct.increment(1);
			}
	...
	public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
		...
		// create job
		String jobName = ClassUtils.getShortClassName(MyCounter.class);

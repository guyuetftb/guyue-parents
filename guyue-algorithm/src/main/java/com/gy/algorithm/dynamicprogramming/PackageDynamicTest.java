 ...
 */
public class PackageDynamicTest {
	...
	public static long package01(List<Long> elementCapacityList, List<Long> valueList, Long finalCapacity) {
		...
		for (int rowElement = 1; rowElement < elementNum; rowElement++) {
			...
			for (int colCapacity = 0; colCapacity <= finalCapacity; colCapacity++) {
				 ...
				 */
				long dontPut = memory.get(rowElement - 1).get(colCapacity);
				capacity = dontPut;

				/**
				 * 容量值必须 大于 要放入的元素容量, 才进行判断
				 */
				if (colCapacity >= elementCapacityList.get(rowElement)) {
					long elementValue = valueList.get(rowElement);
					/**
					 * 在不考虑当前元素, 且总容积减去元素所占容量 情况下, N-1元素所能取得的最大值.
					 */
					long preElementMaxValue = memory.get(rowElement - 1).get(colCapacity - elementCapacityList.get(rowElement).intValue());
				...
				memory.get(rowElement).set(colCapacity, capacity);
			}
		}

		return memory.get(size - 1).get(finalCapacity.intValue());
	}
